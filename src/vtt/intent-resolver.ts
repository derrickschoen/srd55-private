...
export interface PrototypeIntentChoice {
...
export interface PrototypeIntent extends PrototypeIntentChoice {
  readonly fallback: PrototypeIntentChoice | null;
}

export type PrototypeIntentResult =
  | {
      readonly legal: true;
...
function prototypeBranch(choice: PrototypeIntentChoice): EngineIntentBranch {
  return {
    choice: {
      kind: 'attack',
      actionId: choice.action,
      target: { kind: 'combatant', combatantId: choice.targetId as CombatantId },
    },
    movement: {
      willingness: choice.maxMovementFeet === 0 ? 'none' : 'only_if_required',
      maximumFeet: choice.maxMovementFeet,
      opportunityRisk: 'accept_if_needed',
    },
    engagement: { stance: choice.acceptMelee ? 'close_to_melee' : 'maintain_range' },
  };
}

/** Temporary prototype adapter; the MCP protocol itself is replaced in increment 4. */
export function resolvePrototypeIntent(
  state: EncounterState,
  actorId: CombatantId,
  intent: PrototypeIntent,
  resolver: PureIntentResolver = pureIntentResolver,
): PrototypeIntentResult {
  const primary = prototypeBranch(intent);
  const resolution = resolver.resolve(state, {
    actorId,
    ...primary,
    fallback: intent.fallback === null ? null : prototypeBranch(intent.fallback),
  });
  if (resolution.valid) {
    return {
      legal: true,
      resolvedPath: resolution.mechanics.path,
      finalPosition: resolution.mechanics.finalPosition,
    };
  }
  return {
    legal: false,
    refusals: resolution.refusals.map((refusal) =>
      intent.fallback === null ? refusal.summary : `${refusal.branch}: ${refusal.summary}`),
  };
}
