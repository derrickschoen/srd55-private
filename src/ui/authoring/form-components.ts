...
export function createOrderedCardControls(
  ...
  controls.append(
    button('Move up', options.position <= 1, options.onMoveUp),
    button('Move down', options.position >= options.count, options.onMoveDown),
    button('Remove', false, options.onRemove),
  );
  return controls;
}
