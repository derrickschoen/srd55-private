...
describe('directional invariants: dealt', () => {
  it('Devotion dealt > Domination (smite-only) dealt at every level, burst and day', () => {
    // D233: re-measured after substituting the SRD Oath of Devotion. Both
    // builds can smite on round 1, but Sacred Weapon adds Charisma +3 to
    // every attack roll, so Devotion remains strictly ahead in all eight
    // measured cells. Domination's aura-wide Command targeting (L7+) and
    // Blindsight remain deliberately unpriced; see the sim.ts header.
    for (const L of [3, 6, 11, 17] as const) {
      const rngBurst = mulberry32(1000 + L);
      const palBurst = sampleMeanPerRound(devotion, rngBurst, L, 1, 6000);
      ...
      const palDay = sampleMeanPerRound(devotion, rngDay, L, 4, 1500);
...
describe('day vs burst: resource-shape invariants', () => {
  it('devotion day <= burst, per round, at every level (Long-Rest smite queue is finite)', () => {
    for (const L of [3, 6, 11, 17] as const) {
      const rng = mulberry32(4000 + L);
      const burst = sampleMeanPerRound(devotion, rng, L, 1, 6000);
      const day = sampleMeanPerRound(devotion, rng, L, 4, 1500);
  ...
  it('short-rest-only builds (champion, veteran, thief) track day ~= burst within a tight band', () => {
    // Unlike devotion/monk/fiend(Hex)/casters, these builds' resources
