...
(Warrior of the Barbed Court, Veteran, Oath of Domination) against optimized
2024-rules comparator builds, across four character levels and two combat
...
Both forms were verified to pass (71 tests, 7 files) under vitest 4.1.10 and
vitest 3.2.7 in the environment this suite was authored in; nothing in the
suite is version-specific. The first run may create a small `node_modules/`
holding vitest's own transform cache — harmless, gitignore it.
...
4. **Statistical** (`statistical.test.ts`) — tolerance-based checks against
   ...
   and directional invariants (Vengeance Paladin out-damages a
   smite-hoarding Domination Paladin at low levels; a Paladin's day-mode
