...
describe('D233 SRD comparator mechanics', () => {
  it('Champion applies Savage Attacker to one greatsword hit per turn', () => {
    // L3 has five attack slots across four turns (Action Surge adds the fifth).
    // Every attack hits without critting: 2d6(face 5) + ability 3 = 13, for 65.
    // Savage Attacker rolls a second 2d6 pool once on each of the four turns,
    // so the exact draw count is 5 d20 + 10 base d6 + 8 comparison d6 = 23.
    const c = drawCounter(constRng(X));
    expect(champion(c.rng, 3, 1)).toEqual({ dealt: 5 * 13, prevented: 0 });
    expect(c.draws()).toBe(23);
  });

  it('Lore Bard prices Cutting Words as one reduced enemy damage roll per round', () => {
    // L11 burst spends the first four ray volleys: (6+6+5+5)=22 rays.
    // Each ray is 2d6(face 5) + 4d8 CME(face 6) = 34. The standard enemy's
    // three attacks all hit, and the first damage roll each round is reduced
    // by one d10(face 8): 4 * 8 = 32 prevented.
    expect(lore(constRng(X), 11, 1)).toEqual({ dealt: 22 * 34, prevented: 4 * 8 });
  });
});
...
describe('advantage flags carry across round boundaries (F12/F13/F14) and Heroic Warrior fires (F18)', () => {
  ...
  it('championRanged: Vex survives the round boundary (F13)', () => {
    ...
    // hits and sets Vex. Round 0's own draws: 3 misses + 1 hit + 2 damage
    // dice because Savage Attacker compares two weapon-die pools.
    ...
    // Correct total: 6 (round 0) + 2 + 1 (round 1) + 2 + 2 (rounds 2-3) = 13.
    // With Vex re-initialised per round it is 11.
    const HIT = 0.7; // d20=15; hit = 7+wb(1)+2 = 10 -> 25 vs AC 16; crit is 19+
    const values = [
      MISS, MISS, MISS, // round 0 attacks 1-3
      HIT, DAMAGE_DIE_MID, DAMAGE_DIE_MID, // attack 4: two d6=4 pools, +mod5 = 9
...
describe('caster rows are null below their level floor (F21)', () => {
  it('sorcwiz and lore return null at L3 and L6, and a result at L11/L17', () => {
    for (const L of [3, 6] as const) {
      expect(sorcwiz(ALWAYS_MIN, L, 1)).toBeNull();
      expect(lore(ALWAYS_MIN, L, 1)).toBeNull();
    }
    for (const L of [11, 17] as const) {
      expect(sorcwiz(ALWAYS_MIN, L, 1)).not.toBeNull();
      expect(lore(ALWAYS_MIN, L, 1)).not.toBeNull();
    }
  });
});
