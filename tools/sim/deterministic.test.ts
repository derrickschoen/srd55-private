...
describe('constant-value Rng (fixed non-crit hit): exact smite-queue accounting', () => {
  ...
  it('paladin L17 burst: only 3 of 4 rounds can smite (Vow of Enmity blocks round 1)', () => {
    const abil = 5;
    const wb = 3;
    ...
    const smiteTotal = L17_QUEUE.slice(0, 3).reduce((a, b) => a + b, 0) * 6; // d8 face=6, not crit-doubled
    const r = paladin(constRng(X), 17, 1);
    expect(r).toEqual({ dealt: weaponTotal + smiteTotal, prevented: 0 });
  });

  it('paladin L17 day: 12 of 16 rounds can smite, so 3 of the 15 queued entries go unused', () => {
    const perAttack = 19;
    const weaponTotal = perAttack * 3 * 16;
    const smiteTotal = L17_QUEUE.slice(0, 12).reduce((a, b) => a + b, 0) * 6;
    const r = paladin(constRng(X), 17, 4);
    expect(r).toEqual({ dealt: weaponTotal + smiteTotal, prevented: 0 });
  });

  it("domination smite-only L17 burst: round 1 CAN smite (no Vow of Enmity), so all 4 rounds do", () => {
    const perAttack = 19;
