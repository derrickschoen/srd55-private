...
describe('caster day ray totals never exceed the Long-Rest slot budget (F1)', () => {
  ...
  it('same invariant holds for the Lore bard (shares the same RAY_Q mechanism)', () => {
    const rng = mulberry32(4343);
    const burst = sampleMeanPerRound(lore, rng, 17, 1, 4000);
    const day = sampleMeanPerRound(lore, rng, 17, 4, 1000);
    expect(day.mean).toBeLessThan(burst.mean * 0.85);
  });
});
