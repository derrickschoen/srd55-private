-- D203 / CI-4a: externally asserted, name-derived slug keys are a distinct
-- authority class. Existing derived and legacy-opaque rows retain their exact
-- classification; this migration performs no semantic rekey or backfill.
...
CREATE TABLE `__new_catalog_content_identities` (
	...
	CONSTRAINT "catalog_content_identities_key_kind_check" CHECK("__new_catalog_content_identities"."key_kind" IN ('derived', 'asserted', 'bundled-stable', 'legacy-opaque')),
	...
	CONSTRAINT "catalog_content_identities_key_layer_check" CHECK((
        ...
        OR ("__new_catalog_content_identities"."key_kind" = 'bundled-stable' AND "__new_catalog_content_identities"."catalog_layer" = 'bundled')
        OR ("__new_catalog_content_identities"."key_kind" = 'legacy-opaque' AND "__new_catalog_content_identities"."catalog_layer" = 'external')
      ))
);
...
CREATE UNIQUE INDEX `catalog_content_identities_kind_key_unique`
  ON `catalog_content_identities` (`content_kind`,`content_key`);
CREATE INDEX `catalog_content_identities_layer_kind_index`
  ON `catalog_content_identities` (`catalog_layer`,`content_kind`);
CREATE INDEX `catalog_content_identities_name_index`
  ON `catalog_content_identities` (`content_kind`,`normalized_name`);

-- 0033 never mints legacy-opaque for any fresh root. Every root must pass
-- through the asserted/bundled registration seam first.
