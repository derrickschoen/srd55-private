CREATE TEMP TABLE `__characters_sequence_before_optional_features` (
	`seq` integer NOT NULL
);

INSERT INTO `__characters_sequence_before_optional_features` (`seq`)
...
CREATE TABLE `__new_characters` (
	...
	`appearance` TEXT,
	`backstory` TEXT,
	`optional_feature_selections` TEXT DEFAULT '[]' NOT NULL,
	`notes` TEXT,
	...
	CONSTRAINT "characters_optional_feature_selections_check" CHECK(json_valid("__new_characters"."optional_feature_selections")
      AND json_type("__new_characters"."optional_feature_selections") IS 'array'),
...
INSERT INTO `__new_characters` (
  ...
  `revision`, `alignment`, `appearance`, `backstory`,
...
SELECT
  ...
  `wisdom`, `charisma`, `ability_allocation_method`,
  `proficiency_bonus_override`, `rules_edition_preference`, `allow_legacy`,
  `revision`, `alignment`, `appearance`, `backstory`, '[]', `notes`,
...
SET `seq` = max(
	`seq`,
	coalesce(
		(SELECT `seq` FROM `__characters_sequence_before_optional_features`),
		`seq`
	)
)
...
FROM `__characters_sequence_before_optional_features`
WHERE NOT EXISTS (
	SELECT 1 FROM `sqlite_sequence` WHERE `name` = 'characters'
);

DROP TABLE `__characters_sequence_before_optional_features`;
