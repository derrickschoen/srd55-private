...
## 7. r3 amendments (binding; answer Astra's r2 residuals F3/F7/F8/F9/F11 and the F1/F10 notes)

A1 (F1) Evidence paths: the supervisor's diagnostics live in this worktree under `.tmp/runs/quietstone/`
   (`candidate-rgba-oracle.json`, `diag-byid-r1.json`, `diag-exact.json`, `svg-png-oracle.py`); the
   `.claude/consensus/quietstone/` copies exist only in the main checkout.
A2 (F3) Envelope ledger semantics — replace the blanket "at least its pinned value" rule. Each ledger row is
   typed with explicit comparison semantics and full-precision values (integers or rational components,
   ...
   - `nativeDetail`: LOWER bound as the rational { changed, opaque } exactly as the existing
     ...
     of 16384 (see `diag-exact.json`). Assertion `changed * ledger.opaque >= ledger.changed * opaque` (no
     float) AND `changed > 0`; ids absent from the ledger must satisfy the strict `>= 0.04`.
   - `paletteTwinPairs`: EXACT permitted set per id, drawn from the seven pinned palette pairs (F2); assertion:
     the offending pairs actually present in the bitmap equal the row's set (not a subset — a vanished pair
     retires the row); ids absent from the ledger must have no pair under 8.
   - `silhouette`: LOWER bound keyed by the actual archetype PAIR with the full-precision descriptor distance:
     cleric/undead 0.011284722222222224, cleric/beast 0.011835007440476187 (values from the existing
     `sharedBustSilhouette`/`descriptorDistance` on production ids); assertion `distance >= pinned`; every
     other pair `> 0.012`.
   - `lightDirection` (terrain only, procedural classifier `directionalCentroid`): typed outcome per id —
     pillar `{ kind: 'centroids', highlight: 126, shadow: 126 }` (asserted equal to the pinned integers);
     rubble `{ kind: 'no_shadow_pixels', highlightPixels: 207 }`; crate `{ kind: 'no_highlight_no_shadow' }`;
     hazard is procedural and never enters the ledger. The classifier's internal `expect` must be replaced in
     the ledger test by an explicit pixel census (the test may not call a helper that throws).
   - `alphaShadowRegion` and `plateAnnulus` are NOT ledger rows: they are R3/R4 hard assertions.
...
A3 (F7) Probe protocol made literal: `--board-glyphs none` (the repository default); snapshot server on
   ...
   ≥ 0.9 per-class result reported separately for both runs; the −0.05 rule is an evaluation tolerance, not a
   claim of zero regression, and the owner sees the raw per-class table.
A4 (F8) Importer grammar: the delivered SVGs are `<?xml …?><svg xmlns width height viewBox
   shape-rendering="crispEdges"><title/><desc/>` followed only by `<rect …/>` elements. The importer accepts
   exactly that wrapper (validated attributes: width=height=128, viewBox 0 0 128 128) and the inert `<title>`
   and `<desc>` text elements; any other element or drawing primitive is a hard error. Overlapping rectangles
   are REJECTED (the delivery has none — verified by Astra; the importer checks it, and the CI fixture set
   includes an overlap control that must fail). Exact selected-set coverage means the 58 ids remaining after
   excluding `terrain-hazard` (R7) and the `replaces: null` low wall.
A5 (F9) Independent output checks after encoding: a test-side PNG decoder (`tests/helpers/png-decode.ts`,
   `node:zlib` inflate + per-row unfilter for filter types 0–4, IHDR validation) decodes (a) every rendered
   authored PNG from `renderStarterArtPng` and (b) every COMMITTED `public/assets/art/*.png` for the 58 ids, and
   compares the RGBA digest with the frozen external oracle; (c) the preview SVG's `<image id="asset-…"
   href="data:image/png;base64,…">` entries are decoded and matched to the inventory digest per id before the
   preview pin moves; the 34 procedural ids are decoded the same way against their existing pins' rendered
   output. Generator self-equality and regenerated hashes do not substitute for these.
A6 (F10) Registry consistency (index key = art id = input id, complete 58 coverage, no duplicates) is
   validated when the completed registry is assembled in `index.ts`, not inside the per-module constructor,
   so no module depends on the index while it initialises.
A7 (F11) Provenance test compares `request.redoOf` with `` `public/${row.output.path}` ``; the existing
   manifest row-id/input-id equality check is preserved across both source variants.
