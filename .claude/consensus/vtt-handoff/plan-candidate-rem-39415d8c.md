...
**Required order.** Validate door/principal/current/target without side effects; block repump; record prior pause and establish transient cancellation state; abort pending request (`src/combat/coordinator.ts:333-338`); only then await step settlement, resolving the human wait (`src/combat/coordinator.ts:505`; `src/combat/controllers.ts:94-121`; host wait `src/vtt/dm-encounter-host.ts:540`); revalidate; apply/flush world_operation; recompute offers with fresh planner (`src/combat/coordinator.ts:830-837`); restore prior pause policy. Running publishes a fresh offer before door.set settles; paused re-offers on resume. Add no field to cached persisted continuation (`src/combat/coordinator.ts:60-66`, `:725-735`, `:798-799`).
...
**Decision/resources.** Keep the five conformance/parity/config/boundary resources.

**Implementation.** Retain the behavior matrix. Convergence here covers only in-process, Worker and WebSocket adapters entering EncounterSessionService/permitted reducer. It does not inspect encounter-app.ts before S8.

**Gate.** Targeted parity plus `engine-boundary.test.ts -t "all renderer adapters converge on the session service"`; one parity Playwright at 4410; fast typecheck.

**Verification/rollback/assumptions.** Record adapter graph/port0/artifact=dev. Complete convergence waits for S8. Revert five.
