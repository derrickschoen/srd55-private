...
- Token identity is absent from the player shape (`src/combat/visibility.ts:157-163`) and must stay absent: ControllerRequest embeds that view (`src/combat/controllers.ts:29-45`), persisted coordinator stores requests/continuations (`src/combat/coordinator.ts:76-81`, `:319-325`), and journal bytes/checksum include it (`src/vtt/session-persistence.ts:487-488`, `:2324-2335`). D603 requires renderer-only enrichment after filtering.
...
Resource status is explicit: S0a modifies two manifests and adds one test; S0b adds three tools/one test and modifies the package; S1 adds two source, two schema, one test; S2a has four resources (one modified projection plus adapter/map/test; D603 removes the visibility edit); S2b adds builder/generator/two fixtures/test; S2c adds publisher/test and modifies package. S3a is the justified seven-file mechanical exception; S3b/S3c use five files each; S3d has six resources (five code/test files plus the baseline-authored fixture); S3e four. S4/S5/S7a/S7b/S7c/S8/S9a/S9b use five each; S5b/S6c/S9c/S9d/S10b use four; S6a/S6b use five; S7d has two resources; authorized S10a adds five docs.
...
### S2a — D359-safe renderer-only token enrichment

**Decision/resources.** Preserve persisted bytes. Modify `src/vtt/encounter-projections.ts`; add `src/vtt/handoff/asset-id-map.ts`, `src/vtt/handoff/scene-snapshot.ts`, `tests/unit/vtt/scene-snapshot.test.ts`. D603/F24 forces this sole S0-S2 list change: remove `src/combat/visibility.ts`; add no tokenId to PlayerVisiblePlacedCombatant, PlayerView, ControllerRequest or PersistedCoordinatorState. Gate stays unchanged.

**Implementation.** Add visibleCells only to renderer-level PlayerBoardProjection. Session builds a transient `RendererTokenIdentityIndex` from canonical tokens, never returned by `coordinatorState()`/serialization. Snapshot adapter first derives IDs from the filtered player projection, intersects this index, then joins token IDs; omit all others. A snapshot-only classification marks this per-seat. Implement B without accepting EncounterState.
...
**Verification/rollback/assumptions.** Pin centers, two seats, hidden removal and identity. Compare baseline pending-request JSON/checksum before/after snapshot projection and prove enrichment absent. Roll back four files; persistence compatibility is proved, not assumed.
...
### S3d — Deadlock-safe door intent and transient restore freshness

**Decision/resources.** Modify `src/combat/coordinator.ts`, `src/vtt/dm-encounter-host.ts`, `src/vtt/encounter-session-service.ts`, `tests/unit/vtt/door-intent.test.ts`, `tests/unit/vtt/session-persistence.test.ts`; add baseline-authored `tests/fixtures/vtt/pending-request-baseline.v1.json` (six-resource exception for immutable evidence).

**Persistence boundary.** Before code edits, supervisor captures the fixture from unmodified `0f84e09f`, records SHA/checksum and hand-verifies request/revision/pause/continuation. Never regenerate it. Persisted types/bytes remain unchanged. Offer freshness is transient state outside `coordinatorState()`/checksum. Restore marks pending request/continuation stale, preserves cursor/pause, rejects old request, and recomputes/re-offers at current revision. Restored pause remains paused until resume; replay uses persisted commands only.

**Supported semantics and required order.** The only supported blocking triples are open `{movement:false,lineOfSight:false,cover:"none"}` and closed `{movement:true,lineOfSight:true,cover:"total"}`; these fields are mechanically independent (`src/combat/terrain.ts:15-20`). A door in any other current triple returns `UNSUPPORTED`. First validate principal, active-actor policy, door ID/kind, current canonical triple, and requested target without side effects. If already in the requested state, await the existing durability barrier, return `{revision:currentRevision}`, and exit before cancellation with no reducer execution, journal append, or snapshot event.

For an actual change: block repump; record prior pause and establish transient cancellation state; abort the pending request (`src/combat/coordinator.ts:333-338`); only then await step settlement, resolving the human wait (`src/combat/coordinator.ts:505`; `src/combat/controllers.ts:94-121`; host wait `src/vtt/dm-encounter-host.ts:540`); revalidate revision/object/target; then dispatch exactly `{type:"world_operation",actor:activeCombatant,cost:"none",operation:{kind:"modify_object",objectId:door.id,changes:{blocking:targetTriple}}}` and flush. This uses the existing modify-object event path (`src/combat/encounter.ts:11682-11700`). Although `actor:null,cost:"none"` exists (`src/combat/encounter.ts:12489-12499`), v1 policy requires the active combatant as DM attribution and returns `UNSUPPORTED` when none exists. Recompute offers with the fresh planner (`src/combat/coordinator.ts:830-837`), then restore prior pause policy. Running publishes a fresh offer before door.set settles; paused re-offers on resume. Add no field to cached persisted continuation (`src/combat/coordinator.ts:60-66`, `:725-735`, `:798-799`).
...
**Verification/rollback/assumptions.** With a genuinely pending human request and no human response, `door.set` completes in bounds and yields a different current-revision offer. Targeted assertions pin both complete canonical triples, noncanonical-door `UNSUPPORTED` with the original request still pending, and same-state success with unchanged revision plus zero aborts, reducer calls, journal rows, and snapshot events after durability acknowledgment. Also test the exact world-operation payload/active actor/cost, no-active-actor refusal, newly enabled and invalidated moves, stale response, restore/resume/replay/checksum, and unchanged frozen fixture. Roll back six resources.
...
### S7c — Adapter-only three-runtime parity

**Decision/resources.** Add `tests/helpers/vtt-handoff/transport-conformance.ts`, `tests/unit/vtt/runtime-parity.test.ts`, and `tests/browser/vtt-handoff/runtime-parity.spec.ts`; modify `tests/browser/vtt-handoff/playwright.config.ts` and `tests/unit/vtt/engine-boundary.test.ts`.

**Implementation.** Run the same fixed two-room scene, clock, and RNG through each transport. The explicit conformance scenarios are: correlation IDs; lawful structural boundary values and validation failures; authoritative role/player mismatch; initial snapshot; token move result and snapshot; canonical door change and same-state no-op; `light.set` returning `UNSUPPORTED`; every terminal intent outcome; reconnect with full resnapshot; late subscription; visible-to-hidden entity removal; duplicate mutation ID; malformed transport handling beside a legitimate empty ID; subscription/disposal cleanup; and no blind retry after an unknown mutation outcome. Unit tests pair in-process with the Node WebSocket factory. Browser tests pair the real Worker with WebSocket to an ephemeral Node factory, allowlisting the exact port-4410 Origin and supplying the bearer outside the URL.

Canonical output equality is only a secondary check. Hand-written independent assertions pin the literal ground centers from B, hidden token removal rather than retention, player ownership/cross-seat refusal, acknowledged encounter revision, strictly monotonic event sequence independent of revision, and exactly one reducer execution for a duplicate ID. Fixed inputs make divergences reproducible.

The convergence assertion remains intentionally adapter-only here: in-process, Worker, and WebSocket entries must reach `EncounterSessionService` and the permitted reducer chain, with browser/Node imports separated. It does not inspect `encounter-app.ts` before S8.

**Gate.** `npx vitest run --configLoader runner tests/unit/vtt/runtime-parity.test.ts tests/unit/vtt/engine-boundary.test.ts`; `PLAYWRIGHT_PORT=4410 PLAYWRIGHT_WORKERS=1 npx playwright test --config=tests/browser/vtt-handoff/playwright.config.ts runtime-parity.spec.ts`; `npm run typecheck:fast`. The boundary selection is `engine-boundary.test.ts -t "all renderer adapters converge on the session service"`.

**Verification/rollback/assumptions.** Supervisor records the fixed seed/clock, Node port 0, browser port 4410, artifact `dev`, scenario count/outcomes, and adapter-only graph. Roll back five resources. Shared-code parity never replaces the independent assertions; complete UI convergence waits for S8.

### S7d — Supervisor launch probe outside ordinary discovery

**Decision/resources.** Add `tests/integration-supervisor/vtt/node-runtime-launch.launch-test.ts`, `tests/integration-supervisor/vitest.config.ts`.

**Containment/implementation.** Root Vitest recursively includes names ending `.test.ts` (`vitest.config.ts:34`); `.launch-test.ts` does not match, so npm test/test:gate cannot discover it. Dedicated config includes only the exact launch file. Probe creates mode-0600 claims, runs real `npm run serve -- --vtt-runtime --port 0`, authenticates/opens/snapshots, asserts fresh build identity, terminates and proves child cleanup.

**Gate.** Only supervisor, after build: `npx vitest run --configLoader runner --config tests/integration-supervisor/vitest.config.ts tests/integration-supervisor/vtt/node-runtime-launch.launch-test.ts`. Before full Vitest, read-only discovery listing proves absence.

**Verification/rollback/assumptions.** Record exactly one test/fresh artifact. Remove two files; never ordinary Vitest or 4173.
...
**Gate.** Boundary/lifecycle/selectors Vitest, selecting both S8 boundary assertions; one top-down Playwright at 4410; fast typecheck.
...
npm run test:gate
npm run build
npx vitest run --configLoader runner --config tests/integration-supervisor/vitest.config.ts tests/integration-supervisor/vtt/node-runtime-launch.launch-test.ts
