...
The concrete `get_turn_context` builders are:

| Builder/call site | Blind status |
|---|---|
| Planned diagnostic context, lines 2177–2185 | **Wrong**: always sends `intel_mode` |
| Simulated initial context, around line 2312 | **Correct**: omits it for blind |
| Initial live model dispatch/context plumbing around 3743 | Profile-dependent; `fullTurnContextBase` is used |
| Direct initial/runtime construction around 4300–4320 | Correct only where profile branching is retained |
| Advice update reached from line 4767 through `fullTurnContextBase` at 3743 | Advice-only and profile-correct |
| Blind repair context around 4811 | Must remain blind-specific |
| Host correction reconstruction at 5189 | **Structurally unsafe**: no blind guard |
| Subsequent/final context reconstruction around 5757/5815 | The fallback is **wrong** |
| Simulated/recovery variants around 2234 and 2994 | Scripted or reused-launcher paths; readiness semantics differ |
| MCP server-internal retrieval/construction sites | Runtime validated, not statically profile-indexed |

The two real `fullTurnContextBase` callers are line 3743 and line 5189; the former is reached through the advice-only update at line 4767, while the latter follows host authorization correction without a blind guard ([ai-dm-conversation.ts:3743](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:3743), [ai-dm-conversation.ts:4767](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:4767), [ai-dm-conversation.ts:5189](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:5189)). Ordinary blind rejection without a staged proposal bypasses that correction coordinator, but a staged proposal rejected by host authorization can enter it ([ai-dm-conversation.ts:5088](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:5088), [turn-exhaustion-coordinator.ts:219](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/turn-exhaustion-coordinator.ts:219)).

The other profile-divergent tools must receive the same audit: compare the exact DM and blind tool-name/schema lists in `schemas.ts`, then classify every caller by the runtime profile it can receive. Generic MCP resource methods are not members of either advertised engine-tool set and must not satisfy that audit. In particular, the correction path currently renders the ordinary DM proposal protocol—including `engine.submit_round_proposals`—even for a blind staged proposal rejected by host authorization ([turn-exhaustion-coordinator.ts:282](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/turn-exhaustion-coordinator.ts:282), [engine-server.ts:1008](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/mcp/engine-server.ts:1008)).

The proper long-term guard is a profile-indexed surface, for example:
...
A `BlindEngineToolSurface` would then make `{intel_mode: ...}` a compile error for blind `get_turn_context`, while preventing blind code from naming DM-only tools. Runtime strict validation should remain as a trust-boundary check.
...
- contains descriptors whose exact names equal the expected advertised names for that profile;
- contains the expected schemas/descriptors, represented by a canonical hash.

A record with wrong profile, missing names, extra names, malformed content, or a different dispatch ID is invalid and cannot prove readiness.

### 4.2 Preserve live process evidence for all exit paths
...
Extend—not replace—the existing shared result contract so Pi’s `contractEvidence` survives ([pi.ts:104](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/agent-adapters/pi.ts:104), [agent-adapters.SIMULATED.test.ts:641](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tests/unit/vtt/agent-adapters.SIMULATED.test.ts:641)):

```ts
type AgentTurnExit =
  | { readonly kind: 'completed'; readonly response: AgentResponse }
...
interface AgentTurnResult {
  readonly exit: AgentTurnExit;
  readonly sessionId: string | null;
  readonly processEvidence: ProcessEvidence | null;
  readonly contractEvidence?: readonly string[];
  readonly engineCatalogEvidence: EngineCatalogEvidence | null;
}
```

Update Codex’s nonzero path before its current throw so a required-engine initialization failure returns `infrastructure_failed` with preserved evidence. Stderr phrases may be supporting evidence but never the sole classifier.

Claude Code, OpenCode, and Pi use the updated process evidence without acquiring Codex-specific catalog semantics. Pi retains `contractEvidence`. Local OpenAI and simulated adapters return `processEvidence:null` and either `engineCatalogEvidence:null` or deterministic simulated evidence; no existing evidence field is removed.

Change `coldStart` from an always-binding API to:
...
Only a completed dispatch with a real reusable session produces `bound`; timeouts, cancellations, and startup failures remain unbound. Resume preserves an already-existing binding but propagates the new exit union. This replaces the present unconditional completed-result requirement ([agent-session.ts:29](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/agent-session.ts:29), [agent-session.ts:65](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/agent-session.ts:65), [agent-session.ts:280](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/src/vtt/agent-session.ts:280)).
...
- its tool name is exactly a member of the expected tool-name set for the dispatch’s profile;
...
8. `inconclusive` is persisted but triggers an integrity STOP before packet conversion; it is neither scored nor automatically excluded as diagnosed infrastructure.
...
### 4.4 Separate delivery from failure attribution
...
Forbidden-content checks always run over whatever actual ingress exists. Missing required fields become honest incomplete evidence rather than an exception. Incomplete delivery is valid row provenance for `service_null`, timeout/refusal, cancellation, and infrastructure outcomes; outcome attribution is determined separately.

### 4.5 Exhaustive dispatch transitions

Before reading or authorizing any proposal, branch on `AgentTurnResult.exit`:

| Phase | Completed | Cancelled | Timed out | Infrastructure failed |
|---|---|---|---|---|
| Primary | Read only dispatch-correlated spools; authorize only a completed staged proposal | Read readiness/context for audit; do not consume proposal; persist `infrastructure_failed/dispatch_cancelled` | Read proposal only as `stagedUnconsumed`; never authorize; persist `refused/agent_timeout` | Read evidence only; persist `infrastructure_failed`; skip proposal, reducer, correction, and execution |
| Correction | Ordinary DM/advice correction may consume only a completed proposal; blind correction is prohibited | No proposal consumption/default; persist `refused/correction_cancelled` | No proposal consumption/default; persist `refused/correction_timeout` | Persist `infrastructure_failed`; no default |
| Adjustment | Apply only a completed, validated adjustment | Record adjustment cancelled; retain previously authorized baseline | Record adjustment timed out; ignore staged adjustment and retain baseline | Mark whole cell `infrastructure_failed`; stop further execution |
| Speculative | A completed validated result may be considered by existing adoption policy | Discard speculation; primary result unchanged | Discard staged speculation; primary result unchanged | Mark the cell `infrastructure_failed` when joined; never adopt speculative output |

A proposal written before timeout is audit evidence only and is never passed to authorization. Coordinator callbacks must return the same exit union rather than throwing for every non-completed correction, which is the current behavior ([ai-dm-conversation.ts:4725](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:4725), [ai-dm-conversation.ts:4765](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:4765), [ai-dm-conversation.ts:5140](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-conversation.ts:5140)).

Tests must cover timeout before context, timeout after context, timeout after a staged proposal, cancellation after a staged proposal, correction timeout, adjustment timeout with baseline retention, speculative timeout, and startup failure in every phase.
...
- `arena-row-v3`: require catalog evidence, delivery status, configured caps, nullable delivered measurements, scheduled-cell identity, dispatch identity, and outcome-aware ingress evidence.
- Reject partial hybrids.

Extend packet outcome parsing at [ai-dm-rerun-packet.ts:148](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-rerun-packet.ts:148) to accept literal `infrastructure_failed`. Preserve the existing historical audit shape at [ai-dm-rerun-packet.ts:218](/home/vagrant/PhpstormProjects/dnd-wt-blind-dm/tools/ai-dm-rerun-packet.ts:218).
...
- `inconclusive` evidence is rejected before scoring;
...
4. An inconclusive classifier result, absent row, duplicate cell, uncorrelated readiness record, or schema/integrity violation remains a STOP.
...
- All four delivery states through finalization and packet conversion.
- Timeout before context and after staged proposal.
- Exit transitions for primary, correction, adjustment, and speculation.
- `coldStart` bound/unbound behavior.
- Pi `contractEvidence` and other adapters’ existing result contracts.
...
- Raw row → packet/key → scorer exclusion.
...
npx vitest run --configLoader runner \
  ...
  tests/unit/vtt/agent-session.test.ts \
  tests/unit/vtt/engine-server.test.ts \
...
5. Write a provenance manifest containing:
   ...
   - code-identity split: 27 retained hard observations at `90484d453b7b1fe63ed28c0a53570a80e158e6`, three hard replacements at the patched revision, and all brutal observations at the patched revision;
   - D590 and D591 owner decisions;
   - analyzer, validator, runbook, and sidecar hashes and reviews.

6. Run the replacement external validator against the actual mixed 27-old/3-new hard grid and the new brutal grid, including a controlled startup-failure fixture. Run packet/key analysis to prove all 30 scheduled identities remain present and only literal infrastructure pairs are excluded. Do not run the scored pair analysis until the advice arm and new analysis pins have been explicitly approved.
