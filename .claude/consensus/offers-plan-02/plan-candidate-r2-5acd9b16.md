...
Legacy speed/mode writer drift has its own accepted disposition. The current reducer can emit `{40,0,50}` after applying +10 Speed and `{60,0,90}` after granting fixed fly 60 to a Speed-30 active actor because application refreshes once and the operation wrapper then adjusts `remaining` again. Migration retains each triplet byte-for-value, adds no grant, sets `exhaustion:'spendable'`, and records `inference:'legacy_writer_drift'` with the observed fields. These states import successfully at all four raw-state boundaries. Because the bytes do not establish a safe spendable capacity, movement spending, grants, refreshes, and offer/evaluation paths remain typed unresolved for the rest of that old turn; the migrator does not clamp, repair, or call them malformed. Negative/non-finite fields remain malformed and are rejected.

The lossy boundary is explicit. A legacy payload cannot recover authoritative Dash-versus-Flee source provenance or original grant IDs. If its current-turn event segment is absent/truncated, contains an unrecognized writer, or disagrees with the observed capacity, it cannot safely recover the multiplier count; it also cannot resurrect a grant already erased without historical evidence. Every inactive combatant's grant count/source is unresolved because its historical turn base is not recoverable: a structurally valid arithmetic-drift shape retains `inference:'legacy_writer_drift'`, while every other inactive triplet receives `inference:'indeterminate'`. In indeterminate cases migration retains the observed triplet byte-for-value in memory, sets `grantProvenance:{kind:'legacy_unresolved',inference:'indeterminate',...}`, creates no guessed rows, and derives exhaustion only from a complete commanded-Flee segment. No spending or grant-sensitive recomputation is allowed while provenance is `legacy_writer_drift` or `indeterminate`; refusal occurs before changing state or spending a resource. This is the fail-closed rule: uncertain provenance may delay movement for the remainder of an imported old turn, but it never rejects valid old-engine output, silently turns earned movement into zero, repairs contradictory bytes from its own output, or invents movement.

Incoming-turn recovery happens before any boundary callback can refresh movement. Immediately after `startTurn` assigns `activeCombatant` and `round`, a new `beginIncomingTurnMovement` replaces the entire expired prior-turn ledger with `grants:[]`, `grantProvenance:{kind:'known'}`, and `exhaustion:'spendable'` while temporarily retaining the observed numeric triplet. It runs before `revertExpiredWildShapes`, legendary refresh, and `processBoundary`; therefore an expiring movement effect may call `endEffects -> refreshActiveTurnMovement` against known empty provenance. The later existing `startTurnMovement(effectiveSpeed)` remains the authoritative zero-spent reset. A real transition witness imports an inactive `{40,0,50}` `legacy_writer_drift` actor with a +10 movement effect expiring at its next start, ends the prior actor's turn, observes no refusal while the boundary removes the effect, and finishes at `{speed:30,spent:0,remaining:30,grants:[],grantProvenance:{kind:'known'},exhaustion:'spendable'}`. Mutating the invalidation below `processBoundary` must reproduce the refusal.

Saved-session journals use one additional replay contract so provenance migration cannot break complete-state equality. VTT session schema 13 adds required revision field `movementLedgerReplay: {kind:'preledger_v1'} | null`; new revisions write `null`. After verifying every legacy revision checksum and bundle fingerprint, the v12-to-v13 migration walks revisions in parent-before-child order, migrates each snapshot, sets the marker on every pre-ledger revision, recomputes branch fingerprints/checksums/fingerprint, and preserves commands/events/RNG. During `replaySessionRevisions`, a marked `reducer_applied` revision runs the ordinary reducer, then passes only the replayed state through the same `canonicalizePreledgerReplayMovement` routine: current-turn grant events deterministically relabel replay-created `dash`/`command_flee` rows to `legacy_unresolved`, assign the same event-sequence IDs/provenance/exhaustion as snapshot migration, and change no numeric movement or non-movement field. The existing `requireCanonicalEqual(revision.encounterState,replayed.state,'reducer state')`, exact event comparison, and RNG comparison then run unchanged. The routine never reads the expected snapshot to choose values; the checksum-covered marker and replayed event history are its only inputs. Unmarked/current revisions get no canonicalization and retain ordinary exact equality. Export retains old revision markers; revisions appended after import are unmarked.

The journal witness is a checked-in, genuinely linked three-revision v12 bundle: a pre-Dash parent, a `reducer_applied` Dash child, and a `reducer_applied` move-10 child ending at `{60,10,50}`. The test calls `importSavedSession`, then `exportSavedSession` to force full replay, imports that exported schema-13 journal into a new store, exports again to force a second replay, and requires byte-identical first/second exports plus exact revision, command, event, RNG, triplet, grant-count, and marker assertions. Mutations to a non-provenance state field, triplet, multiplier count, command, event, or RNG must still fail replay; replacing post-reducer canonicalization with snapshot-derived expected values must fail a source/ID mutation. Replay verification and its assertions are not weakened or provenance-blind.
...
6A2b (7 files) modifies `src/vtt/session-persistence.ts`, `src/db/migrations.ts`, `src/db/schema.sql`, `tests/unit/schema.test.ts`, `tests/unit/schema-check-constraints.test.ts`, and `tests/unit/db/migrations.test.ts`; it adds `drizzle/0066_vtt_session_movement_ledger.sql`. Saved sessions migrate before replay at `src/vtt/session-persistence.ts:3385-3411`. This sub-slice raises `VTT_SESSION_SCHEMA_VERSION` from 12 to 13, adds required `movementLedgerReplay`, registers the adjacent v12-to-v13 migration, permits schema version 13 in the persisted revision table without narrowing versions 1-12, and owns replay-only canonicalization before unchanged complete-state equality. Schema/migration tests require the exact 1-13 accepted set and reject 0/14/fractional versions. No docs schema changes.

6A2c (10 files) modifies `tests/unit/vtt/{engine-context-integrations,engine-round-session,session-persistence,regret}.test.ts` and adds six hand-authored pre-ledger inputs: `tests/fixtures/vtt/pre-movement-ledger-{one-dash,double-dash,command-flee,speed-writer-drift,mode-writer-drift,multi-revision-dash-journal}.json`. The first five are complete legacy arena-shaped encounter payloads with one active Speed-30 actor and exact current-turn history: one Dash `{60,10,50}`, two Dashes `{90,10,80}`, commanded Flee `{60,60,0}`, speed +10 drift `{40,0,50}`, and fixed fly-60 mode drift `{60,0,90}`. Each of the four raw-state ingress specs imports all five through its real boundary, for 20 adapter witnesses, asserting integrity where applicable, acceptance, exact observed triplet, provenance classification, grant count/source/ID, and exhaustion. Separate derived cases remove/contradict current-turn history and assert indeterminate provenance plus pre-mutation refusal; a corroborated Speed-0 grant remains represented. The sixth fixture is the genuine linked v12 three-revision journal specified above, not an arena snapshot wrapper. `session-persistence.test.ts` performs its full import -> replaying export -> reimport -> replaying export contract and keeps complete replay assertions. These six files increase the checked-in tests-tree pre-ledger fixture inventory from 57 to 63 and are never generated from migration output.
...
- `indeterminate legacy provenance refuses grant-sensitive work before mutation and incoming turn clears before boundary refresh` — treating missing history as known empty, placing invalidation after `processBoundary`, or refusing the complete import must fail; a Speed-0 state with one corroborated grant event separately retains one suppressed row.
...
Rollback: revert 6A2c's six new fixtures and four specs, then 6A2b's schema-13 marker/migration/replay canonicalization and database schema migration, then 6A2a's shared migrator and non-journal ingress adapters, then 6A1's required-field/incoming-turn migration and direct consumers as one tranche; Slice 6B cannot start until all of 6A's cumulative contracts pass. Because no legacy payload is rewritten in place, rollback leaves original saved bytes and capture pins recoverable.
...
| S6A2a-S6A2c | `engine-context-integrations`, `engine-round-session`, `session-persistence`, `regret`, `schema`, `schema-check-constraints`, `db/migrations`, and all six `pre-movement-ledger-*` fixtures (20 raw-state imports, writer drift, exhaustion, schema 13, and genuine journal replay) |
...
|---|---|---|
| Known fresh ledger, exhaustion, and pre-boundary incoming-turn invalidation | `src/combat/movement.ts`, `src/combat/encounter.ts`, `src/vtt/monster-planning-state.ts`, and the seven 6A1 test consumers listed above | Any raw-state migration or Grapple lifecycle work |
...
| Schema-13 journal marker/replay persistence | `src/vtt/session-persistence.ts`; `src/db/migrations.ts`; `src/db/schema.sql`; new `drizzle/0066_vtt_session_movement_ledger.sql`; `tests/unit/schema.test.ts`; `tests/unit/schema-check-constraints.test.ts`; `tests/unit/db/migrations.test.ts` | Saved-session import/export or replay of a ledger-bearing revision |
| Boosted, drift, and replay import witnesses | `tests/unit/vtt/engine-context-integrations.test.ts`; `tests/unit/vtt/engine-round-session.test.ts`; `tests/unit/vtt/session-persistence.test.ts`; `tests/unit/vtt/regret.test.ts`; new `tests/fixtures/vtt/pre-movement-ledger-{one-dash,double-dash,command-flee,speed-writer-drift,mode-writer-drift,multi-revision-dash-journal}.json` | Slice 6A dispatch and Slice 6B start |
...
The first command must report all 20 raw-state boundary imports; exact one-Dash/double-Dash/exhausted-Flee cases; accepted speed/mode drift; indeterminate and negative/non-finite cases; the real pre-boundary turn-transition recovery; and the genuine marked three-revision import/replay/export/reimport/replay witness with unchanged complete-state, event, command, and RNG verification. The second must prove database migration `0066`, accept exactly schema versions 1-13, and reject 0, 14, and fractional values. The third must report the exact printed reachable/path sets, positive `A={(h,a)}`, nonbeneficial `(2,0)` candidate, attack-specific-reach-10 negative control with ordinary OA reach 5/Reaction unavailable, the independent OA-window mutation, and the 10-foot hostile-budget killer. Then run the one cumulative Vitest invocation and static checks already specified for the slice; do not run a full suite, build, or gate in this worktree.
...
Engine authority remains absolute: neither player nor model may supply coordinates, paths, dice, DCs, damage, reducers, or non-offered IDs. Reposition geometry and movement provenance stay internal; Ready records and outcome evidence remain DM-only; the DM semantic export never reaches a player channel. Migration verifies integrity against original bytes before normalization, never rewrites checked-in inputs or capture pins, never guesses a fresh grant source, accepts old writer drift without executing it, and refuses before state/resource mutation when provenance is insufficient. Journal replay canonicalization is checksum-marker- and replay-history-driven, never expected-snapshot-driven, and leaves complete-state equality intact. The only intended persistent side effect is that a successfully imported pre-ledger payload is represented in memory and on its next export with schema-13 ledger/provenance/exhaustion fields and markers on migrated revisions; original import bytes remain untouched.

## Rollback

Rollback follows the slice-local rules above. For the migration repair, revert 6A2c's four import/replay specs and six fixtures, then 6A2b's schema-13 marker/replay and database migration files, then 6A2a's three non-journal adapters and shared migrator, then 6A1's required fields/incoming-turn invalidation as one non-dispatchable tranche; never retain an adapter that inserts empty grants or a marker without its replay canonicalizer. For the geometry repair, revert the Slice 11 candidate implementation and its tests together; no registered offer may refer to it. Any activation rollback is only the Slice 17 policy constant and retains evaluation evidence. No rollback rewrites a user's legacy payload, changes a retained pin, or touches the frozen contracts file.
...
- Marked legacy journal replay canonicalizes provenance from replayed history before, not instead of, the existing exact complete-state comparison; no comparison coordinate is dropped.
- D586.5 manifest provenance is still an activation precondition, not an assumption this plan makes.

## Open owner questions

None. D600 supplies the ruling needed to reopen the plan, and all six accepted Astra round-1 findings now have locally checkable plan resolutions. A missing or invalid pre-design D586.5 manifest remains a stop condition for activation, not an unanswered design question.
