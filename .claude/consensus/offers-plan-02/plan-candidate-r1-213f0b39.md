...
- A read-only recursive JSON inventory found exactly 57 checked-in files containing pre-ledger `speed/spent/remaining` turn movement: 413 occurrences of `{0,0,0}`, one of `{30,0,30}`, four of `{30,30,0}`, and zero boosted-capacity occurrences. `node -e` arithmetic then reproduced the capacity component of the migration witnesses: with corroborating event counts, base 30 maps `{60,10,50}` to one unresolved multiplier, `{90,10,80}` to two, and commanded-Flee `{60,10,50}` to one; recomputation returns the same three triplets.
...
export type TurnMovementGrantProvenance =
  | { readonly kind: 'known' }
  | { readonly kind: 'legacy_unresolved'; readonly inference: 'corroborated_legacy_events' | 'indeterminate';
...
Each grant records one additional current-Speed multiplier, not a captured number of feet. This follows the SRD rule and its explicit reduced-Speed example: Dash's increase equals Speed after modifiers, so Speed 15 plus one Dash is 30 total, not 45 (`docs/srd/full/srd-5.2.1.txt:11589-11600`). Fresh `dash` and `command_flee` source discriminants are reducer-authored and never reconstructed from an action label. The third source is migration-only and explicitly says that old bytes proved a multiplier but not its source. `startTurnMovement` clears grants and returns `grantProvenance:{kind:'known'}`. `spendMovement` retains both fields. Both current grant writers at `src/combat/encounter.ts:6238-6250,12465-12480` call one `grantTurnMovement` helper, so two Dash grants are two independently branded rows rather than a collapsed Boolean. Every current mid-turn movement writer—effect application/removal, movement-mode and speed-modification operations, Grappled application/removal, and commanded-resource exhaustion—calls one recomputation function instead of replacing the triplet itself.

The recomputation is exact. Let `B = effectiveSpeed(state, actor)` and `M = sum(grant.speedMultipliers)`. Set `usableCapacity = B * (1 + M)`, retain `spent = current.spent` byte-for-value even when it exceeds that capacity, and set `speed = usableCapacity`, `remaining = max(0, usableCapacity - spent)`. Grappled's canonical `speed_zero` clause makes `B=0`, so grants remain stored but temporarily produce 0; ending one of several Grapples leaves `B=0`, while ending the last re-derives every multiplier from restored Speed (`docs/srd/full/srd-5.2.1.txt:11693-11702`). Later ordinary Speed reductions and increases re-derive both base and granted movement. A future movement-grant source must add a closed `TurnMovementGrant` variant and thereby receives exhaustive refresh/persistence coverage; no raw `speed +=` writer remains.

Required reducer witnesses are: start the turn Grappled, escape, observe `{speed:30,spent:0,remaining:30,grants:[],grantProvenance:{kind:'known'}}`, then move 5 feet; Dash at Speed 30, move 10 feet, become Grappled, release, and observe `{speed:60,spent:10,remaining:50}` with the same grant ID and known provenance; take two Dashes, move 10 feet, Grapple/release, and restore `{speed:90,spent:10,remaining:80}` with two rows; apply two overlapping Grapples after Dash and prove ending only one stays `{speed:0,spent:10,remaining:0}` while ending the last restores 50; after Dash and 10 feet spent, reduce Speed `30->15` and prove `{speed:30,spent:10,remaining:20}`, then remove the reduction and restore `{60,10,50}`; after Dash and 10 feet spent, increase Speed `30->40` and prove `{80,10,70}`, then remove the increase and restore `{60,10,50}`; and acquire commanded Flee's grant, apply/remove the same +10 effect, and prove capacities `60 -> 80 -> 60` with the same grant ID and unchanged `spent`. These cover Grappled restoration, the SRD's reduction case, increases, and every existing non-Grapple grant/refresh path identified at `src/combat/encounter.ts:1801-1812,2994-3051,6238-6250,10675-10751,12465-12480`.

Pre-ledger migration is intentionally narrower than fresh reducer provenance. It first verifies the original format's hash/fingerprint, then validates every old triplet as finite and non-negative with `remaining = speed - spent` and `spent <= speed`. For the active combatant only, it finds that actor's latest canonical `turn_started` event and counts subsequent historical grant-writer `resource_spent` events using the two exact old reducer literals, `Dash` and `Command: Flee by the fastest available means`; these strings are accepted only as migration evidence and are centralized beside the migrator. Let that count be `E`, canonical current `effectiveSpeed` be `B`, and observed capacity be `S`. Only when the event segment is complete and unambiguous and `S = B * (1 + E)` does migration create exactly `E` deterministically branded `legacy_unresolved` multiplier rows, keyed by their event sequences, and record `inference:'corroborated_legacy_events'`. It never converts either historical string to a fresh `dash` or `command_flee` source. Thus base 30 plus `{60,10,50}` and one grant event carries one row and refreshes to `{60,10,50}`, while `{90,10,80}` plus two grant events carries two and refreshes to `{90,10,80}`. A commanded-Flee `{60,10,50}` carries the same one mechanically correct row but remains source-unresolved. With `B=0`, a complete one-event history still carries one suppressed multiplier because history supplies the count even though capacity arithmetic alone cannot.

The lossy boundary is explicit. A legacy payload cannot recover authoritative Dash-versus-Flee source provenance or original grant IDs. If its current-turn event segment is absent/truncated, contains an unrecognized writer, or disagrees with the observed capacity, it cannot safely recover the multiplier count; it also cannot resurrect a grant already erased by an older movement-mode/speed writer before serialization. Every inactive combatant's old triplet is likewise source-indeterminate because its historical base is not recoverable, but that combatant cannot lawfully spend or refresh movement before its next turn start clears the expired ledger. In those indeterminate cases migration retains the observed triplet byte-for-value in memory, sets `grantProvenance:{kind:'legacy_unresolved',inference:'indeterminate',...}`, and creates no guessed rows. Ordinary spending may consume the retained `remaining`; any Dash/Flee grant, Speed/condition refresh, Grapple restoration, or offer/evaluation path that needs a recomputed capacity returns a typed unresolved/refusal before changing state or spending a resource. The next real `startTurnMovement` safely clears the expired old-turn uncertainty to known empty provenance. A malformed triplet is rejected at import. This is the fail-closed rule: uncertain provenance may delay a grant-sensitive action, but it never silently turns earned movement into zero or invents movement.
...
4. **Bottleneck whole search.** Use a 5-by-2 board whose movement-only blocked regions contain `(0,1),(1,1),(3,1),(4,1)`; these are not opaque `blockedCells`, so `b` still sees the frozen anchor `h`. Hostile `h` at `(0,0)` has Speed 20, reach 5, and one 5-foot melee attack. Actor `b` at `(2,1)` has Speed 5, reach 5, and only Dodge after movement. Speed-30, reach-5 ally `a` at `(4,0)` has no attack. The actor origin is 10 feet from `h`. Its exact adjacent path table is `(1,0):[(1,0)]/5`, rejected because it approaches to 5 feet; `(2,0):[(2,0)]/5`, permitted at equal distance 10; and `(3,0):[(3,0)]/5`, permitted at distance 15. All row-1 alternatives are movement-blocked or the origin, so these are complete. Before movement, `h` has reachable set `{(0,0):0,(1,0):5,(2,0):10,(3,0):15}` and the sole reach-5 attack path is `(0,0)->(1,0)->(2,0)->(3,0)`, attacking `a` at 5 feet. If `b` chooses `(2,0)`, the vacated alcove still permits exactly `(0,0)->(1,0)->(2,1)->(3,0)` for 15 feet, so that candidate has `A={}`. If `b` chooses the gate `(3,0)`, `h`'s complete reachable set is `{(0,0):0,(1,0):5,(2,0):10,(2,1):10}` and exact reach-5 attack-path enumeration is empty: the vacated `(2,1)` alcove is 10 feet from `a`, and movement-only wall `(3,1)` plus occupied gate `(3,0)` prevents re-entry to an attacking origin. Removing only `b`'s endpoint occupancy restores reachable `(3,0)` and exactly four simple attack paths within 20 feet: `(1,0)->(2,0)->(3,0)`/15, `(1,0)->(2,1)->(3,0)`/15, `(1,0)->(2,0)->(2,1)->(3,0)`/20, and `(1,0)->(2,1)->(2,0)->(3,0)`/20. Therefore only `(3,0)` has `A={(h,a)}`, `D=J={}`, vector `(0,2,0)`, and `P=2`; it is the whole-search winner and adds `2/4` over stationary Dodge. The isolated negative control changes only `h`'s reach from 5 to 10. Candidate topology, movement budget, walls, and both actor paths are identical, but from occupied-gate state the exact attack paths are `(1,0)->(2,0)`/10, `(1,0)->(2,1)`/10, `(1,0)->(2,0)->(2,1)`/15, and `(1,0)->(2,1)->(2,0)`/15; `A={}` for every permitted candidate and no bottleneck row exists. A wrong hostile budget of 10 instead of 20 still kills the positive pre-move access and remains the budget mutation; the positive witness separately pins the sole shortest access path at 15.
...
### Slice 6A — required TurnMovement grant migration (three bounded sub-slices)
...
This compile-complete sub-slice adds required `TurnMovement.grants` and `TurnMovement.grantProvenance`, updates both production object-literal constructors (`src/combat/encounter.ts:968`; `src/vtt/monster-planning-state.ts:48-52`), and updates all seven tests that either construct the exact triplet or assert its complete object shape, including `tests/unit/combat/movement.test.ts:156-165` and the Dash shape at `tests/unit/combat/encounter.test.ts:154-166`. It also replaces existing Dash, commanded-Flee, speed-effect, and movement-mode raw triplet writes with `grantTurnMovement`, `recomputeActiveTurnMovement`, `spendMovement`, or `exhaustTurnMovement`, and exposes canonical effective Speed to the migration seam without adding a default. No later slice is needed for either required field to compile.

6A2a (5 files) adds `src/combat/turn-movement-migration.ts` and modifies the four raw-state ingress points `src/vtt/mcp/entrypoint.ts`, `src/vtt/engine-round-session.ts`, `src/vtt/session-persistence.ts`, and `src/vtt/regret/rollout.ts`. Arena fixtures currently enter through `decodeArenaFixture` (`src/vtt/mcp/entrypoint.ts:484-540`); canonical round snapshots cast parsed state at `src/vtt/engine-round-session.ts:134-151`; saved sessions migrate before replay at `src/vtt/session-persistence.ts:3385-3411`; regret captures validate their original hash before returning parsed state at `src/vtt/regret/rollout.ts:65-83`. Every adapter calls the one shared versioned migrator after verifying any hash/fingerprint that binds original bytes. No adapter locally inserts an empty array. The migrator validates the old triplet, carries only an active-turn multiplier count corroborated by the latest turn's reducer event segment and observed capacity as deterministic `legacy_unresolved` rows, and marks all other old provenance indeterminate under the fail-closed rules above. All newly serialized canonical state contains both required ledger fields; in-memory consumers never receive an optional field or use `?? []`.

6A2b (7 files) modifies `tests/unit/vtt/{engine-context-integrations,engine-round-session,session-persistence,regret}.test.ts` and adds the hand-authored, pre-ledger inputs `tests/fixtures/vtt/pre-movement-ledger-{one-dash,double-dash,command-flee}.json`. Each fixture contains a complete legacy arena-shaped encounter payload with one active Speed-30 actor, the exact current-turn reducer event segment, and, respectively, `{speed:60,spent:10,remaining:50}`, `{speed:90,spent:10,remaining:80}`, and `{speed:60,spent:10,remaining:50}` after commanded Flee. The Flee fixture's history identifies its origin for the witness, but migration must still produce `source:'legacy_unresolved'`; no display/event label is promoted to fresh mechanical provenance. Each of the four ingress specs wraps and imports all three checked-in payloads through its real boundary, for 12 adapter witnesses, and asserts the original hash/fingerprint where that boundary has one before normalization plus the exact row count, preserved triplet, unresolved source, deterministic event-sequence IDs, and post-import recomputation. Separate cases derived from those inputs remove the current-turn event segment and set Speed 0; they assert indeterminate provenance plus typed refusal before any grant-sensitive mutation. These inputs increase the checked-in pre-ledger fixture-file inventory from 57 to 60 and are never generated from the migrator's output.

Slice 6A is one non-dispatchable migration tranche: all three sub-slices and their cumulative contracts must pass before 6B or any offer work starts. No VTT offer is registered.

Acceptance/mutation targets:

- `fresh and projected turns contain an empty known grant ledger` — omitting `grants:[]` or `grantProvenance:{kind:'known'}` from either production constructor must fail compilation and the exact projection assertion.
...
- `commanded-Flee legacy imports preserve {60,10,50} without guessing Dash provenance at all four ingresses` — parsing `resource_spent.purpose` into a fresh source discriminant must fail.
- `indeterminate legacy provenance refuses grant-sensitive refresh before mutation and clears at next turn start` — treating missing current-turn history as known empty or refusing the complete import must fail; a Speed-0 state with one corroborated grant event separately retains one suppressed row.
- `malformed legacy triplets are rejected after original-byte integrity verification` — accepting `remaining != speed-spent` or checking only normalized bytes must fail.
...
- `command Flee grant survives plus-ten movement effect apply and removal` — rewriting the triplet without retaining the non-Dash grant must fail.
- `split movement retains spent fifteen and known grants empty` — leaving the existing exact assertion at `{speed:30,spent:15,remaining:15}` without the known-provenance fields or resetting `spent` must fail.

Rollback: revert 6A2b's three new fixtures and four specs, then 6A2a's shared migrator and ingress adapters, then 6A1's required-field migration and direct consumers as one tranche; Slice 6B cannot start until all of 6A's cumulative contracts pass. Because no legacy payload is rewritten in place, rollback leaves original saved bytes and capture pins recoverable.
...
- `bottleneck fixture derives vector zero two zero and reach-ten control emits none` — the complete actor candidates, all hostile reachable sets, and every simple attack path through 20 feet must equal the printed planning probe; changing the hostile one-Speed budget from 20 to 10 feet must fail pre-move access, while changing only reach from 5 to 10 must preserve topology and remove `A` across the search.
...
| S6A2a-S6A2b | `engine-context-integrations`, `engine-round-session`, `session-persistence`, `regret`, and the three `pre-movement-ledger-*` fixtures (shared raw-state ingress migration and 12 boosted imports) |
...
|---|---|---|
| Known fresh ledger plus unresolved legacy provenance | `src/combat/movement.ts`, `src/combat/encounter.ts`, `src/vtt/monster-planning-state.ts`, and the seven 6A1 test consumers listed above | Any raw-state migration or Grapple lifecycle work |
| Shared legacy migrator and every raw-state adapter | new `src/combat/turn-movement-migration.ts`; `src/vtt/mcp/entrypoint.ts`; `src/vtt/engine-round-session.ts`; `src/vtt/session-persistence.ts`; `src/vtt/regret/rollout.ts` | Any imported state may enter an in-memory required-ledger consumer |
| Boosted legacy import witnesses | `tests/unit/vtt/engine-context-integrations.test.ts`; `tests/unit/vtt/engine-round-session.test.ts`; `tests/unit/vtt/session-persistence.test.ts`; `tests/unit/vtt/regret.test.ts`; new `tests/fixtures/vtt/pre-movement-ledger-one-dash.json`, `pre-movement-ledger-double-dash.json`, and `pre-movement-ledger-command-flee.json` | Slice 6A dispatch and Slice 6B start |
...
The ordering is strict: 6A1 -> 6A2a -> 6A2b -> 6B, and Slice 11's complete positive/control proof -> Slice 15 reposition registration. Neither migration nor geometry change authorizes a family default.
...
```sh
npx vitest run --configLoader runner tests/unit/combat/movement.test.ts tests/unit/combat/encounter.test.ts tests/unit/vtt/engine-context-integrations.test.ts tests/unit/vtt/engine-round-session.test.ts tests/unit/vtt/session-persistence.test.ts tests/unit/vtt/regret.test.ts
...
The first command must report all 12 boosted boundary imports plus the one-Dash, double-Dash, Flee, malformed-triplet, and indeterminate-provenance mutations. The second must report the exact printed reachable/path sets, positive `A={(h,a)}`, nonbeneficial `(2,0)` candidate, reach-10 negative control, and the 10-foot hostile-budget killer. Then run the one cumulative Vitest invocation and static checks already specified for the slice; do not run a full suite, build, or gate in this worktree.
...
Engine authority remains absolute: neither player nor model may supply coordinates, paths, dice, DCs, damage, reducers, or non-offered IDs. Reposition geometry and movement provenance stay internal; Ready records and outcome evidence remain DM-only; the DM semantic export never reaches a player channel. Migration verifies integrity against original bytes before normalization, never rewrites checked-in inputs or capture pins, never guesses a fresh grant source, and refuses before state/resource mutation when provenance is insufficient. The only intended persistent side effect is that a successfully imported pre-ledger payload is represented in memory and on its next export with the new required ledger/provenance schema; original import bytes remain untouched.

## Rollback

Rollback follows the slice-local rules above. For the migration repair, revert 6A2b tests/fixtures, then 6A2a adapters/shared migrator, then 6A1 required fields as one non-dispatchable tranche; never retain an adapter that inserts empty grants. For the geometry repair, revert the Slice 11 candidate implementation and its tests together; no registered offer may refer to it. Any activation rollback is only the Slice 17 policy constant and retains evaluation evidence. No rollback rewrites a user's legacy payload, changes a retained pin, or touches the frozen contracts file.
...
- Grant-count carry is allowed only for the active actor when its latest complete reducer event segment and canonical current `effectiveSpeed` corroborate the observed capacity; inactive historical bases and missing/contradictory current-turn histories are explicitly indeterminate, not assumed.
...
None. D600 supplies the ruling needed to reopen the plan, and both D586.74 blockers now have locally checkable plan resolutions. A missing or invalid pre-design D586.5 manifest remains a stop condition for activation, not an unanswered design question.
