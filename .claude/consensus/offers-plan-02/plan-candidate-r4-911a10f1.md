...
OFFERS-PLAN-02 round-4 revision: 2026-09-10  
...
Previous OFFERS-PLAN-02 round-3 SHA-256: `a160ecf7915f0e0e4552be1b5642a8282957c9f3e209cfd56c40b6e07f929a86`  
...
## OFFERS-PLAN-02 round-4 disposition

| Astra round-3 finding | Disposition | Checkable resolution in this revision |
|---|---|---|
| F1 historical spend replay subtracted cost from stale `remaining` instead of re-deriving it from `speed`. | **Accepted and fixed** | The `legacy_v12` spend arm now exactly calls the source formula: validate cost and available reported budget, set `spent' = oldSpent + cost`, then `remaining' = speed - spent'`. Both linked writer journals add a 5-foot movement child and pin state plus `movement_completed`: Speed +10 ends `{40,5,35}`, fly 60 ends `{60,5,55}`. Each continues through replay, export, reimport, and replay. |
...
OFFERS-PLAN-02 round 4 re-derived every `legacy_v12` numeric arm through production functions with one read-only probe. `npx vite-node --config /tmp/offers-plan02-vite.config.ts /tmp/offers-plan02-v12-movement-arms.ts` exited 0 and printed: start `{30,0,30}`; direct drift spends `{40,0,50}+5 -> {40,5,35}` and `{60,0,90}+5 -> {60,5,55}`; cost 55 against reported remaining 50 refused with `Movement spend exceeds the remaining budget.`; Dash `{60,0,60}` then move 5 `{60,5,55}`; commanded Flee final `{60,60,0}` after its movement event reported `spent:45,remaining:15`; raw effect refreshes `speed +10: {40,0,40}->{30,0,30}` and `fly 60: {60,0,60}->{30,0,30}` on removal; a Speed reduction from `{30,10,20}` to effective Speed 5 clamped to `{5,5,0}`; and the operation wrappers produced `{40,0,50}` and `{60,0,90}` before their one-cell moves produced `{40,5,35}` and `{60,5,55}`. Those two moves each emitted exactly `movement_completed` at sequence 8 with `spent:5`, the respective `remaining:35`/`55`, and path `[{column:1,row:0}]`. No test runner, build, fixture writer, or persisted-state mutation was invoked.
...
`legacy_v12` is a closed numeric truth table copied from and probed through the cited v12 reducer behavior, not a second general rules engine. It (1) starts a turn at `{B,0,B}`; (2) validates non-negative finite path cost `C`, refuses when `C > oldRemaining`, then sets `spent' = oldSpent + C` and `remaining' = speed - spent'`—it does not subtract from the possibly drifted `oldRemaining`; (3) implements Dash and commanded Flee by adding current `B` directly to `speed` and `remaining`, with Flee's final exhaustion `{speed:S,spent:S,remaining:0}`; (4) refreshes after effect application/removal to `{speed:B,spent:min(oldSpent,B),remaining:B-min(oldSpent,B)}`, intentionally forgetting prior extra movement; and (5) after a `movement_mode` or `speed_modification` spell's application refresh, performs the old wrapper adjustment `{speed:nextB,spent:refreshedSpent,remaining:max(0,refreshedRemaining+nextB-priorB)}`. Thus the linked +10 writer replays `{30,0,30}->{40,0,40}->{40,0,50}`, then a 5-foot move derives `{40,5,35}`; fixed fly 60 replays `{30,0,30}->{60,0,60}->{60,0,90}`, then derives `{60,5,55}`. An exhaustive switch and mutation tests cover all five source-probed arms; current/unmarked replay uses only the new ledger recomputation.
...
Five checked-in genuine v12 journals exercise this contract. The existing three-revision Dash journal is pre-Dash -> Dash -> move 10, ending `{60,10,50}`. Each linked writer journal is also three revisions: a parent at `{30,0,30}`, the actual `reducer_applied` +10-Speed or fixed-fly-60 cast child ending `{40,0,50}` or `{60,0,90}`, and a subsequent `reducer_applied` one-cell voluntary-move child. The Speed child must end `{speed:40,spent:5,remaining:35}` and carry exactly `{type:'movement_completed',sequence:8,combatant:<actor>,path:[{column:1,row:0}],spent:5,remaining:35}`; the fly child must end `{speed:60,spent:5,remaining:55}` with the identical event except `remaining:55`. Each also pins its command path/cause, token destination `(1,0)`, complete non-movement state, and unchanged RNG. Separate marked pacing journals contain a `turn_skipped` child and a `turn_delayed` child; each asserts exact initiative indices, outgoing inactive provenance, newly entering active provenance, events, coordinator, and RNG as well as movement. Every journal runs `importSavedSession -> exportSavedSession` (first full replay) -> import into a new store -> `exportSavedSession` (second full replay), with byte-identical schema-13 exports and exact revision/marker/state/command/event/RNG assertions. Subtracting movement cost from drifted `oldRemaining` must fail the writer children as `{40,5,45}`/`{60,5,85}` and must also fail their exact event remaining values; a current-numeric substitution must fail their cast children. Omitting canonicalization from either pacing arm must fail its journal. Mutations to any other state field, triplet, multiplier count, command, event, pacing index, or RNG must still fail. Replay equality is neither weakened nor provenance-blind.
...
- `marked speed and mode journals replay v12 numeric semantics through movement` — using current recomputation must fail cast states `{40,0,50}` and `{60,0,90}`; subtracting 5 from stale remaining must fail movement states/events as `{40,5,45}`/45 and `{60,5,85}`/85 instead of exact `{40,5,35}`/35 and `{60,5,55}`/55; skipping either wrapper's second adjustment must also fail.
...
- Locally proved: v12 `spendMovement` validates against reported `remaining` but computes the result from `speed` and cumulative `spent`; the two lawful 5-foot drift spends are exactly `{40,5,35}` and `{60,5,55}`, and the reducer emits the same remaining values in `movement_completed`.
...
None. D600 and D604 supply the rulings needed to continue the plan; all six accepted Astra round-1 findings, all three accepted Astra round-2 findings, and the accepted Astra round-3 historical-spend finding now have locally checkable plan resolutions. A missing or invalid pre-design D586.5 manifest remains a stop condition for activation, not an unanswered design question.
