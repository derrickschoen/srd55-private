...
3. `bundledClassContentKeys()` at `src/rules/class-progression-lookup.ts:187-195`
   still returns exactly `['2024:subclass:eldritch-knight',
   '2024:subclass:arcane-trickster']`, and `hasBundledClassContent()` still
...
> exercised, which was the argument for keeping EK/AT). Eldritch Knight and
> Arcane Trickster retire in the same unit that lands the replacement —
...
"Existing Eldritch Knight/Arcane Trickster slot, access, level-up and multiclass
...
5. **Stale comments, the five sites §9 names**, each verified present by the
   ...
   - `src/builder/level-up.ts:53-59` — "Only two subclasses are seeded (Eldritch
     Knight, Arcane Trickster), so a level-3 refusal would dead-end ten of
