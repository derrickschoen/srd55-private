...
§4 names, while the local seeder's two current rows are instead Eldritch Knight
and Arcane Trickster (`docs/srd/full/srd-5.2.1.txt:40-62`,
`docs/srd/full/srd-5.2.1.txt:101-125`,
...
2. the two override-schedule keys (Eldritch Knight and Arcane Trickster).
...
| Existing Eldritch Knight/Arcane Trickster slot, access, level-up and multiclass tests | **No change except stale global-count comments.** The two definitions and all 40 dense rows remain part of the strict superset (`src/rules/class-progression-lookup.ts:727-765`). |
...
**OQ-1 — Should Eldritch Knight and Arcane Trickster remain bundled after the
