...
docs-only design on the parallel homebrew track. Every designed rule remains
subject to owner approval and playtest.

## 1. Identity

OWNER-APPROVAL: Identity paragraph and ancestry disclosure.

An Oath of Domination paladin speaks the future they believe they were shown
into the present, compelling enemies and arranging allies until the scene
matches that certainty. The oath does not spare its bearer grief, exhaustion,
or horror; it only denies that any of those costs could make the chosen course
wrong. Its repeated visible verb is **command**: in combat the paladin trades
their next strike of divine force for another order, in exploration they seek
and interpret signs, and in social scenes they make obedience sound like the
only remaining future.

The subclass's ancestry is concept-level only; no wording or mechanical form
from any non-SRD ancestry source appears here.

## 2. Tenets of Domination

The following tenets reproduce the binding creative inputs verbatim.
...
## 3. Level 3: Oath of Domination Spells
...
[Oath of Devotion](../../srd/full/srd-5.2.1.txt#L3364)). Oath spell access and
Foretold Authority form the two-feature level-3 bundle. **Level 7 carries two
named features by owner ruling 2026-08-11** (Voice of Domination, moved from 3,
and Aura of Certainty); levels 15 and 20 each grant one feature.

### Level 3: Foretold Authority

OWNER-APPROVAL: Feature name and rules text, including the bonus size, the
duration, the use count, and the drafting assumptions on Foreseen strikes
(declared after the miss is known; the converted hit is not a critical hit).

Your strikes prove the vision, and the proof lends your voice weight. After
you cast *Divine Smite*, you gain a +1 bonus to spell attack rolls and your
spell save DC for 1 minute. The bonus doesn't stack with itself; a new casting
restarts the duration.

In addition, you have already seen some blows land. When you miss with an
attack roll, you can cause it to hit instead. The hit is not a critical hit.
You can use this benefit a number of times equal to your Charisma modifier
(minimum of once), and you regain all expended uses when you finish a Long
Rest.

(A +1d8 smite rider against controlled targets appeared in the first
2026-08-11 redesign batch and was removed the same day by owner ruling:
too complicated.)

### Level 7: Voice of Domination

OWNER-APPROVAL: Voice of Domination rules text as revised by owner ruling
2026-08-11: level-7 placement, the smite-triggered free-action activation, and
Bonus Action casting.

Immediately after you cast *Divine Smite*, you can expend one use of your
Channel Divinity to empower your voice for 1 minute (no action required).
...
OWNER-APPROVAL: Aura of Certainty rules text, including the language ribbon
and its interaction boundary.

While a creature that knows at least one language is in your Aura of
Protection, it understands the literal meaning of any words you speak in a
language you know. This understanding doesn't allow you to understand the
creature in return.
...
> first creature to bend the knee.
...
You can also restore your use of it by expending a level 5 spell slot (no action
required).
...
and a Charisma-modifier-per-Long-Rest pool of miss-to-hit conversions
(Foreseen strikes); it still adds no new attack, Action, or companion. This
follows the paladin posture
...
| Foretold Authority | Entry engine; rock | The +1 is passive after a smite; Foreseen strikes are a Charisma-modifier-per-Long-Rest pool (typically 3) of miss-to-hit conversions, declared after the miss, never critical | No action and no Concentration; the +1 is a 1-minute window | The +1 is live in any fight where Divine Smite is cast; the strikes are strongest concentrated into one decisive fight and thin across a long day | Spell save DC, smite cadence, and Charisma; a 3-level dip gets the pool and can feed it with any class's attacks | C | Sacred Weapon adds Charisma to every weapon attack for 10 minutes; Foretold Authority instead sharpens the spell engine by a bounded +1 and converts a few whole misses — a nova resource where Sacred Weapon is a sustained one |
| Voice of Domination | Tier 2 engine; rock | No at-will effect; one use can produce up to nine slotless castings within 1 minute, with a freely chosen eligible target each time | Activation is a free action taken immediately after casting *Divine Smite* (owner ruling 2026-08-11); each casting uses a Bonus Action; no Concentration | Native Channel Divinity: 2 uses at entry, 3 at level 11; one use recovered on a Short Rest; expected in a consequential fight once a smite has landed | Spell save DC and Paladin Channel Divinity; seven Paladin levels and a smite-cast gate the engine, closing the dip export | C/S | Sacred Weapon lasts 10 minutes, adds Charisma to every attack with one weapon, and rides on the Attack action. Voice lasts 1 minute and adds no damage, but its slotless, freely retargetable control has a higher tactical ceiling; every attempt consumes a Bonus Action and allows the spell's normal save |
...
**Foretold Authority check.** At level 5 with DC 13 against a +2 save, the
+1 DC moves *Command*'s land rate by 5 percentage points. Each Foreseen
strike converts a whole miss: weapon die plus modifier immediately, and —
when no other attack landed that round — it also rescues the round's Divine
Smite from idling. Concentrated into a single four-round fight, the three
conversions are worth roughly +4 damage per round at level 3 in simulation
(the kit's one deliberate overshoot: slightly above a Vow-of-Enmity round
there); spread across a four-fight day they thin to roughly +1 per round,
below the Sacred Weapon estimate of ≈2.55.
...
approximate accuracy gain is 15 percentage points per attack. That is about
`2 × 0.15 × 8.5 = 2.55` added expected damage each round before other hit
riders, for up to 10 minutes and without Concentration. Voice adds `0` damage,
and on every Voice casting round its Bonus Action cannot also carry Divine
Smite.

**Formation ceiling.** At Charisma +5, Foreseen Formation can move five
...
|---|---|---|---|
| Entry (3) | Ten-spell cadence begins; smite-keyed +1 spell attack/save DC and Charisma-modifier miss-to-hit conversions per Long Rest | Ten-spell cadence begins; CD accuracy mode | Sacred Weapon sharpens weapon attacks broadly and sustained; Foretold Authority sharpens the spell engine narrowly and spends a small nova pool on certainty — whole misses become hits. The pool deliberately lets a single decisive fight run slightly hot. No slotless control exists until 7. Playtest amber. |
| Tier 2 (7) | Voice's smite-triggered slotless *Command* engine, plus Charm-save Advantage and one-way comprehension in the aura | Charm immunity in the aura | Two features at 7 by owner ruling 2026-08-11: the slot is deliberately heavy. The aura half stays below the defensive comparator; Voice carries the tactical ceiling and cannot start without a smite. Playtest amber. |
...
- **Ordinary turn:** Attack action; Bonus Action available for Divine Smite,
  ...
  Foretold Authority's windows open and close passively.
- **Voice activation turn (7+):** Attack action with a hit; Bonus Action casts
...
- **Burst turn at 20:** Dominion Foretold's Bonus Action includes its first
  *Command*; the Action can be Attack, but that turn's Bonus Action is already
  spent, so Divine Smite cannot be added.
- **One-fight day:** Channel Divinity and spell slots can be concentrated into
  the encounter. Voice can distribute commands among different targets, but
  each command gets a save and the subclass creates no extra damage carrier.
- **No-Short-Rest day:** two Channel Divinity uses at entry and three from
  Paladin 11 remain available after the Long Rest; the oath spells and aura
  still function when those uses are gone.
- **Rest-rich day:** each Short Rest restores only one native Channel Divinity
  use. Nothing in the subclass restores its own input or extends an activation
  beyond 1 minute.

## 6. Failure-Taxonomy Pass

The distance guard also passes: this is a chosen oath, no feature controls
Undead, no aura adds Charisma to allied melee damage, and no burst-fear effect
or fear-signature loop appears. Its mechanical identity is command, certainty,
and prophecy. Voice of Domination, Aura of Certainty, Foreseen Formation, and
Dominion Foretold were also screened against D178's prohibited-name bar; none
matches.
...
| F3 Dip Bait | A 3-level outsider gets the fixed level-3 spell pair and Foretold Authority. The +1 spell attack/DC exports to a caster stack only for a minute after a slot is smited away, but **Foreseen strikes export to any attacker**: Charisma-modifier guaranteed hits per Long Rest feed GWM/Sharpshooter-style builds and smite novas regardless of the host class (the converted hit is never a critical hit, which caps crit-fisher abuse). Voice sits behind seven Paladin levels and a smite cast, which removes the slotless-control dip this row used to flag. | Amber — the strikes pool is the new hottest dip surface; targeted multiclass playtest required, including a low-Charisma martial dip reading (minimum one use). |
...
| F10 White-Room Day | One-fight, no-Short-Rest, and rest-rich cases are recorded above. Best-round subclass damage is a converted miss (weapon die, modifier, and a rescued smite); the one-fight day is the pool's deliberate best case, and a no-conversion day leaves the full base chassis. The strongest swing remains repeated fresh-save action denial. | Control breadth and duration remain playtest amber. |
...
Legend: **Green** means the draft contains checkable evidence; **Amber** means
the structure is reviewable but table evidence or owner approval is still
required; **Red** means revision is required before review. There are no Red
items in this draft.
...
| Feature-count explanation | Amber | Level 3 has the standard two names: fixed oath spells plus Foretold Authority. Level 7 carries two names — Voice of Domination (moved from 3) and Aura of Certainty — by owner ruling 2026-08-11, deliberately off the one-name schedule; 15 and 20 have one each. |
...
| Checklist item | Status | Evidence |
|---|---|---|
| Level-3 loop present | Green | Land a smite (spending a Foreseen strike if the round would whiff); press spells through the sharpened minute; repeat. From 7, the same smite also unlocks the slotless Command engine. |
...
| Checklist item | Status | Evidence |
|---|---|---|
| Outsider reading of levels 3–5 | Amber | Fighter 17 loses its level-18 feature; Bard 17 delays level-9 magic; Warlock 17 delays invocation and spell progression. Each buys the level-3 spell pair and Foretold Authority after three levels; Voice now requires Paladin 7 and a smite cast, so the slotless engine no longer dips. The exported +1 spell attack/DC is smite-gated and minute-long. |
| Dip-resistant scaling | Amber | No proficiency, armor, attack-stat, or convertible resource grant. The redesign moves Voice behind seven Paladin levels; the remaining dip surface is a smite-gated +1 and the Foreseen strikes pool — the latter is class-agnostic guaranteed accuracy and scales with the dipper's Charisma, so it is the row to watch. |
...
| Near-automatic overshoots absent | Amber | No extra actions or attacks, broad save grant, defense formula, or recovery loop. Foreseen strikes ARE bounded unconditional accuracy — a Charisma-modifier-per-Long-Rest pool of guaranteed non-critical hits; the bound and the never-crit clause are what keep this out of overshoot territory, and playtest must confirm it. |
...
| Extra-economy carrier check | Green | No extra action, attack, or turn exists; a Foreseen strike converts an attack the paladin already made rather than granting a new one; Formation movement cannot carry an Opportunity Attack. |
| Advantage gate | Green | Defensive Charm-save Advantage only, within the base aura, below the comparator's immunity. |
| Flat-bonus gate | Green | The only flat bonus is Foretold Authority's +1 to spell attack rolls and save DC — gated behind casting *Divine Smite*, non-stacking, and capped at 1 minute. No weapon-attack, AC, or save flat bonus exists. |
...
| Frequency statement | Green | Oath spells: scene-dependent but broad; Foretold Authority: the +1 is live in any smite fight, the strikes typically 3/day and strongest saved for one decisive fight; Voice: offered in consequential fights once a smite lands, with up to nine casting turns if the full duration is usable; Aura: passive whenever relevant; Formation: expected 0–2 offers/session; Dominion: one decisive scene/day. |

### 7.6 Resource and rest stress

| Checklist item | Status | Evidence |
|---|---|---|
| Uses model matches effect | Amber | Voice's strong, freely retargetable control spends native CD but no slot; repeated Bonus Actions and fresh saves are its remaining per-casting costs. Foreseen strikes are a Long-Rest pool, matching their nova shape (whole misses become hits — a Short-Rest cadence would over-fuel them). Formation spends native CD; the capstone follows the comparator's Long Rest/level-5-slot reload shape. Live testing must validate that CD alone is sufficient. |
...
| Drawback integrity | Green | Costs are intrinsic Channel Divinity, duration, and Bonus Action costs; no equipment or target choice erases them, and one save has one consequence. |
...
| Foretold Authority | C | — | — | Yes via smiting | Rock |
| Voice of Domination | C | — | S | Yes via smiting | Rock |
...
| Checklist item | Status | Evidence |
|---|---|---|
| Pillar coverage grid | Green | Grid above covers every feature and all three pillars. |
| No single-pillar silence | Green | Fixed divinations, comprehension, Truesight, and *Dream* create player-initiated non-combat presence; no level is ribbon-only. |
| Niche-trespass check | Amber | Controller and social-specialist boundaries are mechanically bounded but require observation from those seats. |
| Campaign-contingency count | Green | Zero core features are gated by creature family, terrain, damage type, or ally condition. |
| Social dependency disclosed | Green | Tyrannical command is the disclosed fantasy; session-zero consent and expectations belong in DM guidance below. |
| Equipment and companion continuity | Green | No equipment requirement and no companion mechanic; Faithful Steed receives flavor only. |

### 7.8 Complexity and table speed

| Checklist item | Status | Evidence |
|---|---|---|
| Bookkeeping count: one tracked quantity maximum | Amber | Three small quantities now exist: Voice's remaining duration, Foretold Authority's +1 window, and the Foreseen strikes pool. Each is a single number or timer and two are per-fight, but the one-quantity claim no longer holds as written; the timed cold-reader test must confirm the load is still light. |
...
| Cold-reader test | Amber | Every action, trigger, range, target, duration, save, limit, recovery, and scaling point is stated; independent adjudication has not occurred. |

### 7.9 Playtest coverage and status

| Checklist item | Status | Evidence |
|---|---|---|
| Usage instrument | Green | Record eligible/noticed/offered/chosen/resolved counts, decline reason, targets chosen, saves and rounds denied, forgotten duration, rule reopenings, and affected-seat participation for every feature. |
| Acquisition test | Amber | Not run; test a new reader at Paladin 3 with both level-3 features. |
| Middle-tier test | Amber | Not run; repeat combat and social scenes at Paladin 7. |
| Untested-altitude flag | Amber | Level 11: untested. Level 17: untested. Levels 15 and 20 are explicitly provisional. |
| Harmed-party observation | Amber | Not run; controller and social-specialist seats must record lost or preserved contributions. |
| Fifth-use verdict | Amber | Not run; use the timing and usage instrument on the fifth Voice sequence. |

### 7.10 Clean-room and release gate

| Checklist item | Status | Evidence |
|---|---|---|
| Permitted citations only | Green | The dispatch authorizes the binding repository inputs and decisions in addition to guidelines and anchored SRD; the draft uses only those sources and cites only guidelines and anchored SRD rules. |
| No external names or Product Identity | Green | No external creator, product, setting, character, creature, or plane name appears outside the mandatory attribution notice. |
| No non-SRD quotation | Green | Quoted game prose is binding original input or original draft prose; SRD-derived rules carry the required notice. |
| Risk disposition | Green | No failed item is hidden; all evidence gaps are Amber and listed below as provisional. |

| Provisional item | Risk | Required test | Owner/status |
|---|---|---|---|
| Foretold Authority | The +1 may make oath-spell control too reliable, and Foreseen strikes may make single-fight novas (guaranteed smite delivery) feel automatic; the L3 burst deliberately runs slightly above the Vengeance comparator in simulation | Acquisition tests at 3 and 5; record strike timing (whiff-rescue vs greed), pool exhaustion, land-rate shift with and without the +1, and one-fight-day feel | Owner approval pending; untested |
...
| Niche boundaries | A controller or social specialist may contribute less | Harmed-seat review in combat and negotiation | Reviewer not assigned |
...
OWNER-APPROVAL: Final prose of the written vision and its three stated readings.

> At the foot of a pale tower, a crowned figure sets the final stone in a wall
> around a sleeping city. Beyond the wall, black water climbs though no rain
> falls. At dawn the water is gone, the crown lies beneath the final stone, and
> every door in the city stands open.

The passage deliberately supports at least three readings:

1. **The paladin's reading:** the wall must be completed, its ruler must accept
   the cost, and the coming ruin will be turned away.
2. **The darker reading:** setting the final stone makes the water rise; by
   dawn the ruler is buried in the work and the open city is empty, conquered,
   or dead.
3. **The benign reading:** the water recedes at dawn without intervention, the
   crown is willingly laid down, and the open doors mark a city that never
   needed a wall.

The vision supplies images and sequence but no causal words. The paladin adds
causality, treats that reading as the only one, and never audits the addition.

OWNER-APPROVAL: Provisional resolution of D186's tenet-title mismatch.

D186 assigns this refusal to re-examine the vision to a tenet titled “What Was
Shown Needs No Second Showing,” but no tenet with that title exists in the
binding final set reproduced in section 2. This draft does not invent or
rewrite a tenth tenet. Pending owner confirmation, **The Vision Does Not Rest**
carries that function: the paladin reads every comforting or warning sign in
one voice and finds no sign that quarrels with another.

## 9. DM Guidance

The following paragraph reproduces the binding guidance:
...
OWNER-APPROVAL: Added DM guidance for using the vision, consent, and rules
adjudication.

- Secretly choose one reading, change which reading seems strongest as events
  develop, or never choose. Preserve the distinction between a true image and
  a fallible interpretation; ambiguity is not permission to make *Augury* or
  *Divination* knowingly false.
- Put the bodily toll after the decision or beside its execution, never before
  it as a pause that could change the outcome. Let witnesses disagree about
  whether the result was rescue or conquest.
- Discuss coercive magic, authoritarian player-character conduct, and
  player-versus-player boundaries before play. The oath's certainty is not
  permission to remove another player's agency outside the game's agreed
  rules.
