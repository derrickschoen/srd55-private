...
> **Design note — how this table is derived, and from what.** Every row is
> generated by one rule: **caster level = one-third your Monk level, rounded
> up; read that row of the SRD 5.2.1 Multiclass Spellcaster table, columns
> 1–4.** (ceil(3/3) = 1 → 2 first-level slots; ceil(7/3) = 3 → 4/2;
> ceil(13/3) = 5 → 4/3/2; ceil(19/3) = 7 → 4/3/3/1.) "Rounded up" is this
> document's design choice: it is what makes spell slots exist at Monk 3, the
> level the subclass arrives. The source is the Multiclass Spellcaster table
> in SRD 5.2.1 ("Multiclassing"), which is CC-BY-4.0 content; no non-SRD
> table was consulted, and anyone can reproduce this table from that source
> and this formula alone. Owner ruling 2026-08-07.
