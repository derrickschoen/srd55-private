# Section 15 — Copyright Notice chain

OGL 1.0a §6 requires that anyone distributing Open Game Content *"update the
COPYRIGHT NOTICE portion of this License to include the exact text of the
COPYRIGHT NOTICE of any Open Game Content You are copying, modifying or
distributing"*, and add their own notice for original Open Game Content they
distribute.

This file is the chain for **exactly what this folder holds**. Any document in
`ogl/` must reproduce the notices below along with the full text of
`OGL-1.0a.txt`.

## Current chain

```
Open Game License v 1.0a Copyright 2000, Wizards of the Coast, Inc.

System Reference Document Copyright 2000-2003, Wizards of the Coast, Inc.;
Authors Jonathan Tweet, Monte Cook, Skip Williams, Rich Baker, Andy Collins,
David Noonan, Rich Redman, Bruce R. Cordell, John D. Rateliff, Thomas Reid,
James Wyatt, based on original material by E. Gary Gygax and Dave Arneson.
```

Both entries are reproduced **exactly** as they appear in the 3.5 SRD's own
`Legal.txt`, which is committed unmodified at `srd-3.5/Legal.txt` so the wording
can be verified rather than trusted.

## Adding to the chain

When a document in this folder distributes original Open Game Content of our
own, §6 requires a new entry with **title, copyright date, and copyright
holder's name**. Append it below the entries above, never in place of them —
the chain is cumulative and order is preserved.

Template:

```
<Document title> Copyright <year>, <copyright holder>; Author <author>.
```

Nothing original has been added yet; the chain is source-only.

## What is Open Game Content in this folder

Per OGL §8 each distributed document must state this for itself. Folder-level
position:

- `srd-3.5/SRD-3.5-rtf.zip`, `srd-3.5/psionic-fist.txt` and
  `srd-3.5/Legal.txt` are the 3.5 SRD as published. All of it is Open Game
  Content except the Product Identity the SRD's own legal file designates, and
  except the text of the licence itself, which that file states *"is not Open
  Game Content."*
- `LICENSING.md`, `README.md` and this file are our own commentary, are **not**
  Open Game Content, and are not offered under the OGL. They quote the licence
  and the SRD's legal file for identification and analysis only.
