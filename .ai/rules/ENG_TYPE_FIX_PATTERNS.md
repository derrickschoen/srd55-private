...
| T4 | **Literal-typed constants.** `Error.name` (or any tag) declared `override readonly name: 'X' = 'X'` — the ANNOTATION, not `as const`, is what makes `""` unassignable. | request.ts:26 (`0e5da577`). | StringLiteral→`""` mutant → TS2322. |
