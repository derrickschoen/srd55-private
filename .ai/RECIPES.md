...
| **Snapshot** (undo/redo) | Covered by `TABLE_SCOPES` for an owned TABLE. For a column on `characters`, `src/character/character-state.ts`: `CHARACTER_STATE_COLUMNS` (`:273`) — BY HAND | a save-point restore test |
...
in `tests/unit/sharing/codec.test.ts`: `PRE_SHEET_WIRE` (`:1671`) — the frozen
