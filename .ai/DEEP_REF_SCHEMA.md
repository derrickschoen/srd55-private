...
| `rows.ts` | The Zod contracts. `COLUMN_REFINEMENTS` (`:341`), `NARROWED_REFINEMENTS` (`:404`), `rowContractError` (`:1241`) |
...
Derived from the classification: `SnapshotTable` (`:967`), `BackupTable` (`:969`),
`ShareTable` (`:970`), and the ordered constants `CHARACTER_STATE_TABLES`
(`:1154`), `DELETE_ORDER` (`:1201`), `BACKUP_TABLES` (`:1265`), `SHARE_TABLES`
(`:1350`).
