...
| `rows.ts` | The Zod contracts. `COLUMN_REFINEMENTS` (`:360`), `NARROWED_REFINEMENTS` (`:426`), `rowContractError` (`:1286`) |
...
Derived from the classification: `SnapshotTable` (`:1016`), `BackupTable` (`:1018`),
`ShareTable` (`:1019`), and the ordered constants `CHARACTER_STATE_TABLES`
(`:1207`), `DELETE_ORDER` (`:1286`), `BACKUP_TABLES` (`:1346`), `SHARE_TABLES`
(`:1436`).
...
path in `src/character/character-state.ts`: `CHARACTER_STATE_COLUMNS` (`:305`) for
