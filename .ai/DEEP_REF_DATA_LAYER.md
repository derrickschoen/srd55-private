...
`vite.config.ts` defines `forbidDrizzleAtRuntime` (`:239`), a rollup plugin that
fails the production build if any module in any entry graph resolves into a
`drizzle-*` package.

**It is registered TWICE** — `vite.config.ts:268` for the main graph and
`vite.config.ts:270` for the worker graph — because Vite builds worker graphs
