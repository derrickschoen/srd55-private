...
import { startMcpClient } from '../../../tools/ai-dm-conversation';
import {
  assertNoImportMeta,
...
function seal(checkout: string, contents: string, recipe: EngineChildRecipe = engineChildRecipe(checkout)) {
  ...
  return writeEngineChildBundle(engineChildBundleDirectory(checkout), recipe, inputs, contents);
}
