...
import {
  ...
  CHROME_FRAME_DARK_INK,
  CHROME_FRAME_LIGHT_INK,
...
function hexRgb(hex: string): readonly [number, number, number] {
  ...
  if (
    ...
    throw new TypeError(`Expected a six-digit hex colour, received ${hex}.`);
  }
  return [
...
describe('D533 creature badge and roster types', () => {
  const roster = [
    {
      id: combatantId('combatant:a'),
      name: 'Reference Fighter',
      kind: 'player_character' as const,
      position: { column: 2, row: 1 },
    },
    {
      id: combatantId('combatant:b'),
      name: 'Chronomancy Abomination',
  ...
  it('assigns unique closed-palette colours and numbers in stable roster order, including a shared cell', () => {
    ...
    expect(badges.map((badge) => badge.color)).toEqual(
      CREATURE_BADGE_COLORS.slice(0, 2),
    );
    expect(badges.map((badge) => badge.stackIndex)).toEqual([0, 1]);
    expect(new Set(badges.map((badge) => badge.number)).size).toBe(
      badges.length,
    );
    expect(new Set(badges.map((badge) => badge.color.id)).size).toBe(
      badges.length,
    );
  ...
  function simulatedRgb(
    ...
    const source = [
      linearChannel(rgb.red),
      linearChannel(rgb.green),
      linearChannel(rgb.blue),
    ] as const;
    const channel = (row: readonly [number, number, number]): number =>
      encodedChannel(
  ...
  function relativeLuminance(color: PaletteColorRef): number {
    const rgb = paletteRgb(color);
    return relativeLuminanceRgb([rgb.red, rgb.green, rgb.blue]);
  }

  function contrastRatio(
    left: PaletteColorRef,
    right: PaletteColorRef,
  ): number {
  ...
  function contrastAgainstRgb(
    color: PaletteColorRef,
    background: readonly [number, number, number],
  ): number {
    const luminances = [
      relativeLuminance(color),
      relativeLuminanceRgb(background),
    ].sort((a, b) => b - a);
    return (luminances[0]! + 0.05) / (luminances[1]! + 0.05);
  }
  ...
  it('uses numerals as the primary identity channel while discs remain distinct under standard dichromacy matrices', () => {
    expect(CREATURE_BADGE_COLORS).toHaveLength(12);
    expect(
      new Set(CREATURE_BADGE_COLORS.map((color) => paletteHex(color.disc)))
        .size,
    ).toBe(CREATURE_BADGE_COLORS.length);
    for (const color of CREATURE_BADGE_COLORS) {
      const sourceRamp: string = color.disc.ramp;
      expect(['cloth-warm', 'cloth-cool'].includes(sourceRamp), color.id).toBe(
        false,
      );
      ...
      if (hsl.saturation > 15) {
        for (const sideHue of BADGE_SIDE_HUES) {
          expect(
            hueDistance(hsl.hue, sideHue),
            `${color.id} against side hue ${String(sideHue)}`,
    ...
    for (let left = 0; left < CREATURE_BADGE_COLORS.length; left += 1) {
      for (
        let right = left + 1;
        right < CREATURE_BADGE_COLORS.length;
        right += 1
      ) {
        for (const [simulation, matrix] of Object.entries(
          DICHROMACY_MATRICES,
        )) {
          const first = cieLab(
            simulatedRgb(CREATURE_BADGE_COLORS[left]!.disc, matrix),
          );
          const second = cieLab(
            simulatedRgb(CREATURE_BADGE_COLORS[right]!.disc, matrix),
          );
          expect(
            deltaE(first, second),
            `${simulation}: ${CREATURE_BADGE_COLORS[left]!.id}/${CREATURE_BADGE_COLORS[right]!.id}`,
          ).toBeGreaterThanOrEqual(10);
        }
      }
    }
    const numeralSignatures = CREATURE_BADGE_COLORS.map((color, index) => {
      const art = renderCreatureBadgeBitmap(
        index + 1,
        color.disc,
        color.numeralInk,
      );
      return art.numeralRows.join('\n');
    });
    expect(new Set(numeralSignatures).size).toBe(CREATURE_BADGE_COLORS.length);
  });

  it('keeps both chrome keyline inks on every badge and ring with a 3:1 contrast escape against floor and roster grounds', () => {
    const grounds = [
      hexRgb('#111113'),
      ...[1, 2, 3, 4].map((step) => {
        const rgb = paletteRgb(ramp('stone', step as 1 | 2 | 3 | 4));
        return [rgb.red, rgb.green, rgb.blue] as const;
      }),
    ];
    for (const background of grounds) {
      expect(
        Math.max(
          contrastAgainstRgb(CHROME_FRAME_DARK_INK, background),
          contrastAgainstRgb(CHROME_FRAME_LIGHT_INK, background),
        ),
      ).toBeGreaterThanOrEqual(3);
    }
    for (const color of CREATURE_BADGE_COLORS) {
      const badge = renderCreatureBadgeBitmap(1, color.disc, color.numeralInk);
      const ring = renderCreatureRingBitmap(color.disc);
      for (const art of [badge, ring]) {
        expect(art.svg, color.id).toContain(paletteHex(CHROME_FRAME_DARK_INK));
        expect(art.svg, color.id).toContain(paletteHex(CHROME_FRAME_LIGHT_INK));
      }
    }
  });

  it('keeps every badge disc distinct from every HP-band ink', () => {
    const hpInks = new Set(Object.values(HP_BAND_INK).map(paletteHex));
    for (const color of CREATURE_BADGE_COLORS) {
      expect(hpInks.has(paletteHex(color.disc)), color.id).toBe(false);
    }
  });

  it('renders every roster word at 4.5:1 or better against the roster background', () => {
    const background = hexRgb('#111113');
    for (const [word, ink] of Object.entries(ROSTER_WORD_INKS)) {
      expect(contrastAgainstRgb(ink, background), word).toBeGreaterThanOrEqual(
        4.5,
      );
    }
  });
  ...
  it('roster_numeral_low_contrast: gives every badge numeral at least 4.5:1 contrast against its disc', () => {
    for (const color of CREATURE_BADGE_COLORS) {
      expect(
        contrastRatio(color.disc, color.numeralInk),
        color.id,
  ...
  it('wraps full 2x roster names without losing glyphs and hyphenates overlong words at a fixed count', () => {
    ...
    const reconstructed = layout.lines
      .join(' ')
      .replaceAll('- ', '')
      .replaceAll(' ', '');
    expect(reconstructed).toBe(
      normalizeLabelText('Chronomancysupercalifragilistic Hero').replaceAll(
        ' ',
        '',
      ),
    );
  ...
  it('keeps both badge slots disjoint from every corner glyph box and the HP bar', () => {
    ...
    for (const badge of badges) {
      for (const [index, glyph] of glyphs.entries())
        expect(overlaps(badge, glyph), `badge vs glyph ${String(index)}`).toBe(
          false,
        );
      expect(overlaps(badge, hp), 'badge vs HP bar').toBe(false);
    }
    for (const stackIndex of [0, 1] as const) {
      const geometry = creatureBustRingGeometry(stackIndex);
      const ring = {
        x: geometry.inset,
        y: geometry.inset,
        width: geometry.size,
      ...
      expect(geometry.size).toBe(CREATURE_BUST_RING_SIZE_PX[stackIndex]);
      expect(
        renderCreatureRingBitmap(
  ...
  it('refuses to reuse a palette colour or overfill a two-badge cell column', () => {
    const tooMany = Array.from(
      { length: CREATURE_BADGE_COLORS.length + 1 },
      (_, index) => ({
        id: combatantId(`combatant:palette-${String(index)}`),
        name: `Foe ${String(index)}`,
        kind: 'monster' as const,
        position: { column: index, row: 0 },
      }),
    );
    expect(() => assignCreatureBadges(tooMany)).toThrow(
      'closed creature-badge palette',
    );
    expect(() =>
      assignCreatureBadges([
        ...roster,
        {
          id: combatantId('combatant:c'),
          name: 'Third',
          kind: 'monster' as const,
          position: { column: 2, row: 1 },
        },
...
describe('HP band parity with the prose classifier', () => {
  function encounterAt(hitPoints: readonly number[]): EncounterState {
    const mover = playerProfile('chrome-pc', {
      hitPoints: 20,
      initiativeBonus: 20,
    });
    const foe = monsterProfile('chrome-foe', {
      hitPoints: 20,
      initiativeBonus: -20,
    });
    ...
    return {
      ...rolled,
      combatants: rolled.combatants.map((combatant, index) => ({
        ...combatant,
        hitPoints: hitPoints[index] ?? combatant.hitPoints,
      })),
    };
  }
  ...
  it('carries actor-knowledge bands onto the DM board and maps them to the four bar bands', () => {
    for (const [pc, foe, expectedPc, expectedFoe] of [
      ...
      for (const [index, expected] of [expectedPc, expectedFoe].entries()) {
        ...
        const projected = board.combatants.find(
          (combatant) => combatant.id === subject.profile.id,
        );
        expect(projected?.hitPointBand).toEqual(
          hitPointKnowledge(state, subject),
        );
...
function serialize(node: InteractiveTestElement): Serialized {
  ...
  return {
    ...
    attributes: [...node.attributes.entries()].sort(([a], [b]) =>
      a.localeCompare(b),
    ),
    text: node.textContent,
    src: styled.src ?? null,
    style: [...(styled.style?.declarations.entries() ?? [])].sort(([a], [b]) =>
      a.localeCompare(b),
    ),
...
const CHROME_CLASSES = new Set([
  'encounter-coordinate-labels',
  'encounter-token-underlay',
...
function withoutChrome(node: Serialized): Serialized {
  return {
    ...
    children: node.children.filter(
      (child) => !CHROME_CLASSES.has(child.className),
    ),
  };
}
...
function inlinePixels(node: InteractiveTestElement, property: string): number {
  const match = node
    .getAttribute('style')
    ?.match(new RegExp(`(?:^|;)${property}:(-?[0-9.]+)px(?:;|$)`, 'u'));
  if (match?.[1] === undefined)
    throw new Error(`${node.className} has no ${property} pixel style.`);
...
function drawGeneratedSvg(
  ...
  for (const rectangle of rectangles) {
    ...
    bitmap.rect(
      left + x * BOARD_CHROME_ART_SCALE,
      top + y * BOARD_CHROME_ART_SCALE,
      width * BOARD_CHROME_ART_SCALE,
      BOARD_CHROME_ART_SCALE,
      neutral(0),
    );
  }
...
function paintOpaque(source: Bitmap, target: Bitmap): void {
...
function changedBustPixels(composition: Bitmap, bust: Bitmap): number {
  let changed = 0;
  for (let y = 0; y < bust.height; y += 1) {
    for (let x = 0; x < bust.width; x += 1) {
      const pixel = bust.get(x, y);
      if (pixel.alpha === 0) continue;
      if (paletteHexKey(composition.get(x, y)) !== paletteHexKey(pixel))
        changed += 1;
    }
  }
  return changed;
...
describe('renderBoard: DM board with chrome, player board without', () => {
  ...
  const projection: EncounterBoardProjectionShape = {
    ...
    combatants: [
      {
        id: combatantId('combatant:fighter'),
        name: 'Reference Fighter',
      ...
      {
        id: combatantId('combatant:cleric'),
        name: 'Reference Cleric',
        kind: 'player_character',
        position: { column: 2, row: 4 },
        life: 'dying',
        hitPointBand: { kind: 'perceived_band', band: 'near_death' },
      },
      {
        id: combatantId('combatant:wizard'),
        name: 'Reference Wizard',
        kind: 'player_character',
        position: { column: 1, row: 2 },
        life: 'living',
        hitPointBand: { kind: 'perceived_band', band: 'bloodied' },
      },
      {
        id: combatantId('combatant:training-brute'),
        name: 'Training Brute',
        kind: 'monster',
        position: { column: 6, row: 3 },
        life: 'living',
        hitPointBand: { kind: 'unknown' },
  ...
  const provenance = {
    revision: 3,
    round: 1,
    stateDigest: 'digest-under-test',
  };

  it('draws one unique badge and one full-name roster row per creature in stable number order', () => {
    const dm = interactiveElement(
      renderBoard(projection, new Set(), null, provenance),
    );
    expect(dm.getAttribute('data-board-chrome')).toBe('on');
    expect(dm.getAttribute('data-coordinate-labels')).toBe(
      COORDINATE_CONVENTION,
    );
    ...
    expect(roster.map((row) => row.getAttribute('data-roster-order'))).toEqual([
      '1',
      '2',
      '3',
      '4',
    ...
    expect(
      new Set(badges.map((badge) => badge.getAttribute('data-badge-number')))
        .size,
    ).toBe(badges.length);
    expect(
      new Set(badges.map((badge) => badge.getAttribute('data-badge-color')))
        .size,
    ).toBe(badges.length);
    for (const [index, combatant] of projection.combatants.entries()) {
      const badge = badges.find(
        (candidate) =>
          candidate.getAttribute('data-combatant-id') === combatant.id,
      );
      const row = roster.find(
        (candidate) =>
          candidate.getAttribute('data-combatant-id') === combatant.id,
      );
      expect(badge?.getAttribute('data-anchor-column')).toBe(
        String(combatant.position.column),
      );
      expect(badge?.getAttribute('data-anchor-row')).toBe(
        String(combatant.position.row),
      );
      ...
      expect(row?.getAttribute('data-badge-color')).toBe(
        badge?.getAttribute('data-badge-color'),
      );
      ...
      const rosterBadge = row?.querySelector(
        '.encounter-roster-badge',
      ) as StyledElement | null;
      expect(rosterBadge?.src, `${combatant.name} roster badge`).toBe(
        tokenBadge?.src,
      );
      const coordinate = row?.querySelector('.encounter-roster-coordinate');
      expect(coordinate?.getAttribute('data-coordinate')).toBe(
        `(${String(combatant.position.column)},${String(combatant.position.row)})`,
      );
      ...
      expect(name?.getAttribute('style')).toBe(
        `width:${String(expectedName.cssWidth)}px;height:${String(expectedName.cssHeight)}px`,
      );
      const bar = dm
        .querySelectorAll('.encounter-hp-bar')
        .find(
          (candidate) =>
            candidate.getAttribute('data-combatant-id') === combatant.id,
        );
      expect(bar?.getAttribute('data-hp-band')).toBe(
        hpBandOf(combatant.hitPointBand),
      );
      const glyph = dm
        .querySelectorAll('.encounter-life-glyph')
        .find(
          (candidate) =>
            candidate.getAttribute('data-combatant-id') === combatant.id,
        );
    ...
    expect(roster.map((row) => row.getAttribute('data-full-name'))).toEqual(
      projection.combatants.map((combatant) => combatant.name),
    );
    expect(
      dm
        .querySelectorAll('.encounter-hidden-ring')
        .map((ring) => ring.getAttribute('data-combatant-id')),
    ).toEqual(['combatant:training-brute']);
    expect(dm.querySelectorAll('.encounter-roster-hidden')).toHaveLength(1);

    const labels = dm.querySelector('[data-coordinate-labels]');
    expect(labels?.querySelectorAll('[data-axis="column"]')).toHaveLength(
      projection.bounds.columns * 2,
    );
    expect(labels?.querySelectorAll('[data-axis="row"]')).toHaveLength(
      projection.bounds.rows * 2,
    );
    ...
    expect(legend?.getAttribute('style')).toContain(
      `min-height:${String(legendHeightPx({ combatants: projection.combatants, objects: projection.worldObjects ?? [] }))}px`,
    );
    ...
    expect(
      legend
        ?.querySelectorAll('.encounter-legend-item')
        .map((item) => item.getAttribute('data-legend-key')),
    ).toEqual(noneEntries.map((entry) => entry.key));
    for (const band of HP_BANDS)
      expect(
        noneEntries.some(
          (entry) => entry.key === `hp-${band.replaceAll('_', '-')}`,
        ),
      ).toBe(true);
  ...
  it('composites identity chrome below every 128-pixel bust so no head or shoulder pixel is covered', () => {
    const styles = readFileSync('src/vtt/styles.css', 'utf8');
    expect(styles).toMatch(/\.encounter-token-underlay \{ z-index: 3; \}/u);
    expect(styles).toMatch(/\.encounter-token \{[\s\S]*?z-index: 5;/u);
    const dm = interactiveElement(
      renderBoard(projection, new Set(), null, provenance),
    );
    const underlay = dm.querySelector('.encounter-token-underlay');
    expect(
      underlay?.querySelectorAll('.encounter-creature-badge'),
    ).toHaveLength(projection.combatants.length);
    expect(
      underlay?.querySelectorAll('.encounter-creature-bust-ring'),
    ).toHaveLength(projection.combatants.length);
    expect(
      dm
        .querySelector('.encounter-token-chrome')
        ?.querySelectorAll('.encounter-creature-badge'),
    ).toHaveLength(0);

    for (const [index, archetype] of TOKEN_ARCHETYPES.entries()) {
      const stackIndex = index % 2 === 0 ? 0 : 1;
      const color = CREATURE_BADGE_COLORS[index]!;
      const ringGeometry = creatureBustRingGeometry(stackIndex);
      ...
      drawGeneratedSvg(under, ring.svg, ringGeometry.inset, ringGeometry.inset);
      drawGeneratedSvg(
        under,
        badge.svg,
        CREATURE_BADGE_LEFT_PX,
        CREATURE_BADGE_TOP_PX + stackIndex * CREATURE_BADGE_STACK_PITCH_PX,
      );
      paintOpaque(bust, under);
      expect(changedBustPixels(under, bust), `${archetype} underlay`).toBe(0);

      const wrongOrder = new Bitmap(CHROME_TILE_PX, CHROME_TILE_PX);
      paintOpaque(bust, wrongOrder);
      drawGeneratedSvg(
        wrongOrder,
        ring.svg,
        ringGeometry.inset,
      ...
      expect(
        changedBustPixels(wrongOrder, bust),
        `${archetype} overlaid chrome`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it('keeps objects and doors in an unclipped typed rail with distinct frames, coordinates, and matching sigils', () => {
    ...
    const door = {
      id: worldObjectId('world-object:rail-door'),
      name: 'Rail Door',
      kind: 'door' as const,
    ...
    const liveObjectLabel = liveDm.querySelector(
      '.encounter-world-object-label',
    );
    expect(liveObjectLabel?.getAttribute('data-label-style')).toBe(
      OBJECT_LABEL_STYLE,
    );

    const snapshotDm = interactiveElement(
      renderBoard(objectProjection, new Set(), null, provenance, 'full', true),
    );
    ...
    const objectSigil = objectTag?.querySelector(
      '.encounter-object-tag-sigil',
    ) as StyledElement | null;
    const legendSigil = snapshotDm
      .querySelector('[data-legend-key="object"]')
    ...
    expect(
      snapshotDm.querySelector('.encounter-world-object-label'),
    ).toBeNull();
    expect(objectTag?.getAttribute('data-label-style')).toBe(
      OBJECT_LABEL_STYLE,
    );
    ...
    expect((objectText as StyledElement | null)?.getAttribute('style')).toBe(
      `width:${String(expectedObjectText.cssWidth)}px;height:${String(expectedObjectText.cssHeight)}px`,
    );
    expect(objectTag?.parentElement?.className).toBe(
      'encounter-object-tag-rail',
    );
    expect(creatureRow?.getAttribute('data-label-style')).toBe(
      CREATURE_LABEL_STYLE,
    );
    expect(objectTag?.getAttribute('data-label-style')).not.toBe(
      creatureRow?.getAttribute('data-label-style'),
    );
    expect(snapshotDm.querySelectorAll('.encounter-object-tag')).toHaveLength(
      8,
    );
    ...
    expect(
      doorTags.map((tag) => [
        ...
        tag
          .querySelector('.encounter-door-tag-text')
    ...
    expect(
      doorTags.every(
        (tag) => tag.getAttribute('data-label-style') !== OBJECT_LABEL_STYLE,
      ),
    ).toBe(true);
    for (const tag of doorTags) {
      const key =
        tag.getAttribute('data-door-state') === 'open'
          ? 'door-open'
          : 'door-closed';
      const railSigil = tag.querySelector(
        '.encounter-door-tag-sigil',
      ) as StyledElement | null;
      const matchingLegend = snapshotDm
        .querySelector(`[data-legend-key="${key}"]`)
        ?.querySelector(
          '.encounter-legend-swatch-mark',
        ) as StyledElement | null;
      expect(railSigil?.src, key).toBe(matchingLegend?.src);
    }
    expect(
      snapshotDm.querySelectorAll('.encounter-world-object-sigil'),
    ).toHaveLength(8);
    const expectedLegendHeight = legendHeightPx({
      combatants: projection.combatants,
      objects: objectProjection.worldObjects,
    });
    expect(
      snapshotDm.querySelector('.encounter-legend')?.getAttribute('style'),
    ).toContain(`min-height:${String(expectedLegendHeight)}px`);
    expect(
      boardChromeDimensions(projection.bounds, {
        combatants: projection.combatants,
        objects: objectProjection.worldObjects,
      }).height,
    ).toBe(
      boardChromeDimensions(projection.bounds).height +
        expectedLegendHeight -
        LEGEND_HEIGHT_PX,
    );
  });
  ...
  it('renders exact 2x roster bitmaps and unique in-cell badges over 200 seeded rosters', () => {
    ...
    for (let sample = 0; sample < 200; sample += 1) {
      ...
      const combatants = cells.slice(0, rosterSize).map((cell, index) => ({
        id: combatantId(
          `combatant:rendered-${String(sample)}-${String(index)}`,
        ),
        name:
          index % 3 === 0
            ? `Long Rendered Creature ${String(sample)} ${String(index)}`
            : `Foe ${String(index)}`,
        kind: 'monster' as const,
        position: {
      ...
      const board = interactiveElement(
        renderBoard(
          {
            bounds,
            combatants,
      ...
      for (const badge of badges) {
        ...
        if (id === null)
          throw new Error('Rendered badge has no combatant identity.');
        const column = numericData(badge, 'data-anchor-column');
        const row = numericData(badge, 'data-anchor-row');
        const rect = {
          x: inlinePixels(badge, 'left'),
          y: inlinePixels(badge, 'top'),
          width: inlinePixels(badge, 'width'),
          height: inlinePixels(badge, 'height'),
        };
        const cellRect = {
      ...
      for (const [index, row] of rosterRows.entries()) {
        ...
        if (combatant === undefined)
          throw new Error('Roster row exceeds its source roster.');
        expect(row.getAttribute('data-full-name')).toBe(combatant.name);
        expect(row.getAttribute('data-roster-order')).toBe(String(index + 1));
        expect(
          row
            .querySelector('.encounter-roster-coordinate')
        ...
        expect(image?.getAttribute('style')).toBe(
          `width:${String(expected.cssWidth)}px;height:${String(expected.cssHeight)}px`,
        );
      }
    }
  });
  ...
  it('stacks two same-cell badges in number order without overlap', () => {
    ...
    const board = interactiveElement(
      renderBoard(shared, new Set(), null, provenance, 'full', true),
    );
    const badges = board.querySelectorAll('.encounter-creature-badge');
    expect(
      badges.map((badge) => [
        badge.getAttribute('data-badge-number'),
        badge.getAttribute('data-stack-index'),
      ]),
    ).toEqual([
      ['1', '0'],
      ['2', '1'],
    ...
    expect(inlinePixels(badges[0]!, 'left')).toBe(
      COORDINATE_GUTTER_PX + 3 * CHROME_TILE_PX + CREATURE_BADGE_LEFT_PX,
    );
    expect(
      inlinePixels(badges[1]!, 'top') - inlinePixels(badges[0]!, 'top'),
    ).toBe(CREATURE_BADGE_STACK_PITCH_PX);
    const rings = board.querySelectorAll('.encounter-creature-bust-ring');
    expect(rings.map((ring) => ring.getAttribute('data-stack-index'))).toEqual([
      '0',
      '1',
    ]);
    for (const [index, ring] of rings.entries()) {
      const stackIndex = index === 0 ? 0 : 1;
      const geometry = creatureBustRingGeometry(stackIndex);
      const cellLeft = COORDINATE_GUTTER_PX + 3 * CHROME_TILE_PX;
      const cellTop = COORDINATE_GUTTER_PX + 2 * CHROME_TILE_PX;
      const ringRect = {
        x: inlinePixels(ring, 'left'),
        y: inlinePixels(ring, 'top'),
        width: inlinePixels(ring, 'width'),
        height: inlinePixels(ring, 'height'),
      };
      expect(ringRect).toEqual({
        x: cellLeft + geometry.inset,
        y: cellTop + geometry.inset,
        width: geometry.size,
        height: geometry.size,
      });
      expect(ringRect.x + ringRect.width).toBeLessThanOrEqual(
        cellLeft + CHROME_TILE_PX,
      );
  ...
  it('uses identical bust and badge composition in live and snapshot DM modes', () => {
    const live = interactiveElement(
      renderBoard(projection, new Set(), null, provenance, 'full', false),
    );
    const snapshot = interactiveElement(
      renderBoard(projection, new Set(), null, provenance, 'full', true),
    );
    expect(
      live.querySelectorAll('.encounter-token-sprite').map(serialize),
    ).toEqual(
      snapshot.querySelectorAll('.encounter-token-sprite').map(serialize),
    );
    expect(
      live.querySelectorAll('.encounter-creature-badge').map(serialize),
    ).toEqual(
      snapshot.querySelectorAll('.encounter-creature-badge').map(serialize),
    );
  ...
  it('contains no non-integer CSS scale in declarations targeting rendered board elements', () => {
    const boardStyles = readFileSync(
      new URL('../../../src/vtt/styles.css', import.meta.url),
      'utf8',
    );
    const board = interactiveElement(
      renderBoard(projection, new Set(), null, provenance, 'full', true),
    ...
    const visit = (node: InteractiveTestElement): void => {
      for (const className of node.className
        .split(/\s+/u)
        .filter((value) => value !== '')) {
    ...
    visit(board);
    const applicableScales = [
      ...boardStyles.matchAll(/([^{}]+)\{([^{}]*)\}/gu),
    ].flatMap((rule) => {
      ...
      const targetsRenderedBoardElement =
      ...
      return [
        ...declarations.matchAll(/scale\(\s*(-?[0-9]+(?:\.[0-9]+)?)\s*\)/gu),
      ].map((match) => ({
    ...
    const everyDeclaredScale = [
      ...boardStyles.matchAll(/scale\(\s*(-?[0-9]+(?:\.[0-9]+)?)\s*\)/gu),
    ].map((match) => Number(match[1]));
    expect(
      everyDeclaredScale.filter((value) => !Number.isInteger(value)),
      everyDeclaredScale.join(','),
    ).toEqual([]);
  });
  ...
  it('keeps every declared rendered chrome pixel in the generated closed palette with no CSS-rounded chrome', () => {
    const closedPalette = new Set([
      ...PALETTE_RAMPS.flatMap((name) =>
        RAMP_STEPS.map((step) => paletteHex(ramp(name, step))),
      ),
    ...
    expect(chromeRoots).toHaveLength(4);
    const visit = (node: InteractiveTestElement): void => {
      ...
      if (source?.startsWith('data:image/svg+xml;charset=utf-8,') === true) {
        ...
        for (const color of svg.match(/#[0-9a-f]{6}/gu) ?? []) {
          expect(
            closedPalette.has(color),
            `${node.className} SVG ${color}`,
          ).toBe(true);
        }
  ...
  it("D525: 'none' and 'light' keep the D516 rows and swap only the light rows; the glyph modes add the room default", () => {
    for (const mode of ['none', 'light'] as const) {
      for (const roomDefault of LIGHT_LEVELS) {
        ...
        for (const presence of [NOTHING_PRESENT, EVERYTHING_PRESENT]) {
          ...
          expect(
            new Set(keys).size,
            `${mode}/${roomDefault} keys are unique`,
          ).toBe(keys.length);
          ...
          expect(keys.slice(5, 5 + lightRows.length)).toEqual(
            lightRows.map((entry) => entry.key),
          );
          ...
          expect(lightRows.map((entry) => entry.key).slice(0, 3)).toEqual([
            'bright',
            'dim',
            'darkness',
          ]);
    ...
    expect(
      lightLegendEntries('none', 'bright').map((entry) => [
        entry.style,
        entry.label,
      ]),
    ).toEqual([
    ...
    for (const mode of ['light', 'full'] as const) {
      expect(
        lightLegendEntries(mode, 'darkness').map((entry) => [
          entry.style,
          entry.label,
        ]),
      ).toEqual([
      ...
      expect(lightLegendEntries(mode, 'dim').at(-1)?.label).toBe(
        'No glyph = DIM',
      );
    }
    // 'none' has no room-default row: the tints name every level whatever the majority.
    expect(lightLegendEntries('none', 'darkness')).toEqual(
      lightLegendEntries('none', 'bright'),
    );
    ...
    expect(fullEntries.find(({ key }) => key === 'side-foe')?.label).toBe(
      'Foe warm-red floor plate',
    );
  ...
  it('leaves the player board byte-identical to the DM board minus its chrome', () => {
    ...
    const dm = serialize(
      interactiveElement(renderBoard(projection, new Set(), null, provenance)),
    );
    expect(
      player.children.some((child) => CHROME_CLASSES.has(child.className)),
    ).toBe(false);
    expect(
      player.attributes.some(([name]) => name === 'data-board-chrome'),
    ).toBe(false);
    const dmWithoutChrome = withoutChrome({
      ...dm,
      attributes: dm.attributes.filter(
        ([name]) =>
          ![
            'data-board-chrome',
            'data-coordinate-labels',
            'data-encounter-rows',
    ...
    const playerComparable = {
      ...player,
      attributes: player.attributes.filter(
        ([name]) => name !== 'data-board-audience',
      ),
    };
    expect(JSON.stringify(dmWithoutChrome)).toBe(
      JSON.stringify(playerComparable),
    );
    expect(
      player.children.every((child) => child.className === 'encounter-cell'),
    ).toBe(true);
    expect(player.children).toHaveLength(
      projection.bounds.columns * projection.bounds.rows,
    );
  });
