...
function record(): PlanRelevanceRecord {
  return {
    policy: 'plan-relevance-v2-composite',
    openMonsterActorIds: [monster],
    proposals: [{
      actorId: monster,
      valid: true,
      resolvedActions: ['main:attack:claw'],
      resolvedTargets: [player],
      refusalCodes: [],
...
describe('composite plan relevance materiality policy', () => {
  ...
  it('wake rule 3: detects a proposal signature change and ignores path drift in the same bucket', () => {
    const changed: PlanRelevanceRecord = {
      ...record(),
      proposals: record().proposals.map((proposal) => ({
        ...proposal,
        resolvedTargets: [other],
      })),
    };
    const pathOnly = replaceRelevant(record(), monster, { position: { column: 1, row: 0 } });

    expect(compare(changed).reasonCodes).toContain('PROPOSAL_RESOLUTION_CHANGED');
    expect(compare(pathOnly).reasonCodes).not.toContain('PROPOSAL_RESOLUTION_CHANGED');
  });
  ...
  it('uses resolver-backed projections while HP damage alone remains deliberately immaterial', async () => {
    const beforeState = freshMonsterPlanningState(await loadArenaFixture('tests/fixtures/arena-basis/seed-3943001.json'));
  ...
  it('canonicalizes open actors, proposals, conditions, and turn events before hashing', async () => {
