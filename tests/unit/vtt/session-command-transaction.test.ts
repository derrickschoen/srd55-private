...
function recordNamedDraw(rng: TransactionalRollRng, path: string): void {
  const component = rng.beginComponent({
    occurrenceId: rollOccurrenceId(`test:${path}`),
...
describe('session command trial core', () => {
  it('retains the exact supplied RNG without owning a fork or checkpoint and concatenates reducer events', () => {
    const fixture = pendingReaction();
    let forks = 0;
    let checkpoints = 0;
    const random = Object.assign(mulberry32(58_410_108), {
      fork: (): SerializableRng => {
        forks += 1;
        return mulberry32(0);
      },
      checkpoint: (): never => {
        checkpoints += 1;
        throw new Error('The core must not checkpoint its RNG.');
      },
    });
    const commandLog: EncounterCommand[] = [];
    const eventBatches: ReturnType<EncounterCommandReducer>[] = [];
    const reducer: EncounterCommandReducer = (state, command, rng, options) => {
      expect(rng).toBe(random);
      commandLog.push(command);
      const reduced = reduceSessionEncounter(state, command, rng, options);
      eventBatches.push(reduced);
      return reduced;
    };
    const core = SessionCommandTrialCore.begin({
      state: fixture.beforeOffer,
      random,
      reducer,
    ...
    expect(applied).toBe(snapshot.state);
    expect(commandLog.map((command) => command.type)).toEqual([
      'move',
      'resolve_pending_decision',
    ]);
    expect(snapshot.events).toEqual(eventBatches.flatMap((batch) => batch.events));
    expect(snapshot.revisionDelta).toBe(2);
    expect(snapshot.fallbackResolutions).toEqual([
  ...
  it('caught automatic-work failure preserves the last accepted apply batch', () => {
    const fixture = pendingReaction();
    const sentinel = new Error('automatic reducer sentinel');
    const createTrial = () => {
      const owner = RollProvenance.recording(transactionalRng(mulberry32(58_410_109)));
      const random = owner.asRng();
      ...
      const reducer: EncounterCommandReducer = (state, command, rng, options) => {
        expect(rng).toBe(random);
        commands.push(command.type);
        recordNamedDraw(random, command.type);
      ...
      const core = SessionCommandTrialCore.begin({
        ...
        boundaryScope: 'initiative_segment',
        accounting: {
          attempted: (): void => { attempted += 1; },
          completed: (): void => { completed += 1; },
        },
      });
      return { owner, random, reducer, commands, core, counts: () => ({ attempted, completed }) };
    };

    const direct = createTrial();
    expect(() => direct.core.apply({
      type: 'move', actor: fixture.moverId, path: [{ column: 2, row: 0 }], cause: 'voluntary',
    })).toThrow(sentinel);
    expect(direct.core.currentState()).toBe(fixture.beforeOffer);
    expect(direct.core.snapshot()).toEqual({
      state: fixture.beforeOffer,
      events: [],
      revisionDelta: 0,
      fallbackResolutions: [],
    ...
    expect(direct.commands).toEqual(['move', 'resolve_pending_decision']);
    expect(direct.counts()).toEqual({ attempted: 2, completed: 2 });
    expect(direct.owner.trace().attempts).toHaveLength(2);
    expect(direct.owner.trace().committedDraws).toHaveLength(2);

    const sessionTrial = createTrial();
    const outcome = runSessionCommandTransaction({
      state: fixture.beforeOffer,
      random: { fork: () => sessionTrial.random },
      reducer: sessionTrial.reducer,
      boundaryPolicy: { kind: 'unattended', askDefault: 'decline' },
      guidance: null,
      boundaryScope: 'initiative_segment',
      ...
      try {
        const rejected = commands.apply({
          type: 'move', actor: fixture.moverId, path: [{ column: 2, row: 0 }], cause: 'voluntary',
        });
        expect(rejected).not.toBe(fixture.beforeOffer);
      } catch (error: unknown) {
        expect(error).toBe(sentinel);
      }
      expect(commands.currentState()).toBe(fixture.beforeOffer);
      return 'caught';
    });
  ...
  it('does not invoke the reducer or completion hook when the attempted hook fails', () => {
    const state = simpleState();
    const sentinel = new Error('attempted accounting hook sentinel');
    let reducerCalls = 0;
    let completed = 0;
    const core = SessionCommandTrialCore.begin({
      state,
      random: mulberry32(58_410_110),
      reducer: (current) => {
        reducerCalls += 1;
      ...
      boundaryScope: 'initiative_segment',
      accounting: {
        attempted: (): never => { throw sentinel; },
        completed: (): void => { completed += 1; },
      },
    });

    expect(() => core.apply({ type: 'roll_initiative' })).toThrow(sentinel);
