...
describe('lineage-chosen character sharing', () => {
  ...
  it('embeds content-v2 in v20 and restores the exact level-5 High Elf choice in both directions', async () => {
    ...
    expect(shareDocumentToPositional(exported)[1]).toBe(20);
    ...
    if (encoded.kind !== 'encoded') throw new Error('Expected a v20 fragment.');
    const decoded = await decodeShareFragment(encoded.fragment);
    expect(shareDocumentToPositional(decoded)[1]).toBe(20);
  ...
  it('decodes the pre-name v17 shape, emits a named v17 fallback, and restores the exact choice after library import', async () => {
    ...
    if (encoded.kind !== 'encoded') throw new Error('Expected a v20 reference-only document.');
    const decoded = await decodeShareFragment(encoded.fragment);
