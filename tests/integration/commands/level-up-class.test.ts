...
describe('level_up_class', () => {
  ...
  it('applies an offered subclass content key at level 3, and refuses a foreign one', () => {
    ...
    // Arcane Trickster belongs to the Rogue: a key of another class must
    // fail loudly, not attach.
    expect(() =>
      levelUp({
        class_definition_id: classId('Fighter'),
        target_level: 3,
        subclass_content_key: '2024:subclass:arcane-trickster',
      }),
  ...
  it('resolves a newly selected subclass by logical locator before its source row exists', () => {
    ...
    const locator = {
      ...
      rule_key: 'eldritch-knight-cantrips',
      ordinal: 1,
    };
    const planned = new LevelUpPlannedEligibleSpells(db).search({
      character_id: characterId as never,
      ...
      class_definition_id: classId('Fighter') as never,
      target_class_level: 3 as never,
      subclass_content_key: '2024:subclass:eldritch-knight' as never,
      locator: locator as never,
      query: '',
    });
    const offered = planned[0];
    if (offered === undefined) {
      throw new Error('Eldritch Knight offered no Wizard cantrip.');
    }
      ...
      db.scalar(
        `SELECT slot.id FROM spell_selection_slots AS slot
         ...
         WHERE source.character_id = ? AND source.source_type = 'subclass'
           AND subclass.content_key = '2024:subclass:eldritch-knight'
           AND slot.rule_key = 'eldritch-knight-cantrips'
           AND slot.ordinal = 1`,
        [characterId],
      ),
    );
